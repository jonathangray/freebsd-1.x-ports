char *ckxv = "UNIX Communications support, 5A(110), 26 Jun 93";
...
ttclos(foo) int foo; {			/* Arg req'd for signal() prototype */
    ...
    if (xlocal) ioctl(ttyfd, TIOCNMODEM, &x);
#endif /* ultrix */
    if (xlocal) {
...
/*
  Can't set speed to zero on AIX 3.1 or 3.1 on RS/6000 64-port adapter,
...
#ifdef AIXRS
    if (ioctl(ttyfd, TXDELCD, "xon") < 0 && errno != EINVAL)
      debug(F100,"hardflow TXDELCD (xon) error", "", 0);
    if (ioctl(ttyfd, TXADDCD, "rts") < 0 && errno != EBUSY)
...
#ifdef AIXRS
      if (ioctl(ttyfd, TXDELCD, "rts") < 0 && errno != EINVAL)
	return(-1);
      if (ioctl(ttyfd, TXADDCD, "xon") < 0 && errno != EBUSY)
	return(-1);
#endif /* AIXRS */
...
*/
	    ...
	    dest[i++] = n & ttpmsk;
#endif /* PARSENSE */
	    if (xfrcan && ((n & 0x7f) == xfrchr)) { /* Check cancellation */
...
conchk() {
...
#ifndef OXOS
#ifdef BSD44
    x = ioctl(0, FIONREAD, &n);         /* 4.4 BSD */
    debug(F101,"4.4BSD conchk","",n);
    return((x < 0) ? 0 : n);
#else
#ifdef PROVX1
...
#ifdef CK_POLL
    {
      ...
      con.revents = 0;
      status = poll(&con, 1, 0);
      if ((status > 0) && (con.revents & POLLIN))
	return(1);
    }
#endif /* CK_POLL */
    return(0);
