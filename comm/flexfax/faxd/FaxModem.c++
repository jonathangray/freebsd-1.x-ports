/*	$Header: /a/cvs/386BSD/ports/comm/flexfax/faxd/FaxModem.c++,v 1.1 1993/08/31 23:42:19 ljo Exp $
/*
    ...
    , revQueryCmd(c.revQueryCmd)
{
    ...
    resetCmds = conf.resetCmds		// prepend to insure our needs
	      ...
	      ;
    /*
     * If the Hayes ATI4 query command is available, the
     * baud rate selection routine will use it to select
     * the best available DCE-DTE communication rate.
     * This string is the actual set of commands sent
     * along with the query command.
     */
    if (conf.hayesQueryCmd != "")
	ati4Cmds = conf.echoOffCmd
		 | conf.verboseResultsCmd
		 | conf.resultCodesCmd
		 | conf.noAutoAnswerCmd
		 | conf.onHookCmd
...
static const AnswerMsg answerMsgs[] = {
    { "FAX",	    3,	FaxModem::OK,		FaxModem::CALLTYPE_FAX },
    { "DATA",	    4,	FaxModem::OK,		FaxModem::CALLTYPE_DATA },
    { "CONNECT FAX",11,	FaxModem::OK,		FaxModem::CALLTYPE_FAX },
    { "CONNECT",    7,	FaxModem::OK,		FaxModem::CALLTYPE_DATA },
    { "NO ANSWER",  9,	FaxModem::NOANSWER,	FaxModem::CALLTYPE_ERROR },
    { "NO CARRIER", 10,	FaxModem::NOCARRIER,	FaxModem::CALLTYPE_ERROR },
    { "NO DIALTONE",11,	FaxModem::NODIALTONE,	FaxModem::CALLTYPE_ERROR },
    { "ERROR",	    5,	FaxModem::ERROR,	FaxModem::CALLTYPE_ERROR },
};
...
FaxModem::findAnswer(const char* s)
{
	...
	if (streq(s, answerMsgs[i].msg, answerMsgs[i].len))
	    return (&answerMsgs[i]);
    return (NULL);
}
...
FaxModem::answerCall(AnswerType atype, fxStr& emsg)
{
	...
	/*
	 * If the receive rate is different from what we are
	 * using, switch before starting up the receive machinery.
	 */ 
	if (conf.recvRate[ctype] != BR0 && conf.recvRate[ctype] != rate)
	    (void) setBaudRate(conf.recvRate[ctype]);
    }
    return (ctype);
}
...
/*
 * Parse an ATI4 status string and return the best
 * DTE-DCE communication rate available.  These bit
 * definitions were taken from Version P2.0 of
 * ``The Technical Reference for Hayes Modem Users''.
 */
static void
parseATI4r1(const char* cp, BaudRate maxRate, BaudRate& rate)
{
    static struct {
	short    b;		// byte in rX string
	short    m;		// bit mask on byte
	BaudRate rate;		// tty data rate
    } i4bits[] = {
	{ 13, 0x2, FaxModem::BR57600 },
	{ 12, 0x1, FaxModem::BR38400 },
	{ 10, 0x1, FaxModem::BR19200 },
	{  9, 0x1, FaxModem::BR9600 },
	{  7, 0x1, FaxModem::BR4800 },
	{  6, 0x1, FaxModem::BR2400 },
	{  5, 0x1, FaxModem::BR1200 },
    };
#define	NI4BITS	(sizeof (i4bits) / sizeof (i4bits[0]))
    u_int n = strlen(cp);
    for (u_int i = 0; i < NI4BITS; i++) {
	short b = i4bits[i].b;
	char c = cp[b];
	if (b >= n || !isxdigit(c) || i4bits[i].rate > maxRate)
	    continue;
	short w = isdigit(c) ? c - '0' : 10 + (c - (isupper(c) ? 'A' : 'a'));
	if ((w & i4bits[i].m) != 0) {
	    rate = i4bits[i].rate;
	    break;
	}
...
/*
 * Reset the modem and select the best DTE-DCE rate.
 */
fxBool
FaxModem::selectBaudRate(BaudRate maxRate, FlowControl i, FlowControl o)
{
    rate = BR0;
    iFlow = i;
    oFlow = o;
    if (ati4Cmds != "" && setBaudRate(BR1200, i, o)) {
	/*
	 * Reset the modem and use the Hayes query command
	 * at 1200 baud to get the bit map of supported DTE
	 * rates.  If the modem doesn't support this query,
	 * then we fall back to a brute force search.
	 */
	if (atCmd(conf.softResetCmd, AT_OK, conf.resetDelay))
	    pause(600);			// in case processing done after "OK"
	else
	    pause(conf.resetDelay);	// usually 2.6 secs
	if (!setBaudRate(BR1200)) {
	    server.reopenDevice();
	    setBaudRate(BR1200);
	}
	flushModemInput();		// flush anything from the modem
	if (atCmd(ati4Cmds, AT_NOTHING)) {
	    ATResponse r;
	    while ((r = atResponse(rbuf,3*1000)) == AT_OTHER)
		if (rbuf[0] == 'r' && rbuf[1] == '1')
		    parseATI4r1(rbuf, maxRate, rate);
	    flushModemInput();
	    if (rate != BR0 && reset(5*1000))
		return (TRUE);
	}
    }
    for (rate = maxRate; rate >= BR1200; rate--)
	if (reset(5*1000))
	    return (TRUE);
    return (FALSE);
}
...
fxBool
FaxModem::lockBaudRate(BaudRate br, FlowControl i, FlowControl o)
{
    rate = br;
    iFlow = i;
    oFlow = o;
    return setBaudRate(br, i, o);
}
...
fxBool FaxModem::sendSetupParams(TIFF* tif, Class2Params& params, fxStr& emsg)
    { return server.sendSetupParams(tif, params, emsg); }
...
FaxModem::atResponse(char* buf, long ms)
{
    ...
    else if (streq(buf, "OK", 2))
	lastResponse = AT_OK;
    else if (streq(buf, "NO CARRIER", 10))
	lastResponse = AT_NOCARRIER;
    else if (streq(buf, "NO DIAL", 7))		// NO DIALTONE or NO DIAL TONE
	lastResponse = AT_NODIALTONE;
    else if (streq(buf, "NO ANSWER", 9))
	lastResponse = AT_NOANSWER;
    else if (streq(buf, "ERROR", 5))
	lastResponse = AT_ERROR;
    else if (streq(buf, "CONNECT", 7))
	lastResponse = AT_CONNECT;
    else if (streq(buf, "RING", 4))
	lastResponse = AT_RING;
    else if (streq(buf, "BUSY", 4))
	lastResponse = AT_BUSY;
    else if (streq(buf, "PHONE OFF-HOOK", 14))
