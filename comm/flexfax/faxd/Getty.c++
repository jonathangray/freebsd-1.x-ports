/*	$Header: /a/cvs/386BSD/ports/comm/flexfax/faxd/Getty.c++,v 1.1 1993/08/31 23:42:36 ljo Exp $
/*
...
extern "C" {
    void closelog(void);
    void openlog(const char* ident, int lgopt, int facility);
};
#endif

#include <stdarg.h>
extern "C" void vsyslog(int, const char*, va_list ap);	// XXX
...
Getty::Getty(const fxStr& l, const fxStr& s, u_int t, fxBool nl)
    : line(l)
    , speed(s)
{
    pid = 0;
...
void Getty::setNologin(fxBool b)	{ nologin = b; }
...
Getty::run(int fd, const char* args)
{
	...
	setupArgv(args);
	if (nologin && access(_PATH_NOLOGIN, F_OK) == 0) {
	    do {
		sleep(1);
	    } while (access(_PATH_NOLOGIN, F_OK) == 0);
	}
	/*
