/*	$Header: /a/cvs/386BSD/ports/comm/flexfax/faxd/Class2.c++,v 1.1 1993/08/31 23:42:03 ljo Exp $
/*
...
Class2Modem::atResponse(char* buf, long ms)
{
	...
	} else if (streq(buf, "+FCON", 5))
	    lastResponse = AT_FCON;
	else if (streq(buf, "+FPOLL", 6))
	    lastResponse = AT_FPOLL;
	else if (streq(buf, "+FDIS:", 6))
	    lastResponse = AT_FDIS;
	else if (streq(buf, "+FNSF:", 6))
	    lastResponse = AT_FNSF;
	else if (streq(buf, "+FCSI:", 6))
	    lastResponse = AT_FCSI;
	else if (streq(buf, "+FPTS:", 6))
	    lastResponse = AT_FPTS;
	else if (streq(buf, "+FDCS:", 6))
	    lastResponse = AT_FDCS;
	else if (streq(buf, "+FNSS:", 6))
	    lastResponse = AT_FNSS;
	else if (streq(buf, "+FTSI:", 6))
	    lastResponse = AT_FTSI;
	else if (streq(buf, "+FET:", 5))
	    lastResponse = AT_FET;
    }
    return (lastResponse);
}
