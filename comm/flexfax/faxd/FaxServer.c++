/*	$Header: /a/cvs/386BSD/ports/comm/flexfax/faxd/FaxServer.c++,v 1.1 1993/08/31 23:42:30 ljo Exp $
/*
...
#if SYS_FCHMOD && !defined(__bsdi__) && !defined(svr4) && !defined(BSD4_4) && !defined(__linux__)
...
FaxServer::answerPhone(AnswerType atype, fxBool force)
{
	...
	if (force || modem->waitForRings(ringsBeforeAnswer)) {
	    fxStr emsg;
	    log = new FaxMachineLog(canonicalizePhoneNumber(FAXNumber));
	    /*
...
FaxServer::restoreStateItem(const char* b)
{
	...
	*cp = '\0';
	...
	recvFileMode = (int) strtol(value, 0, 8);
    else if (streq(tag, "devicemode"))	
	deviceMode = (int) strtol(value, 0, 8);
	...
	modemConfig.parseItem(tag, value);
}
