/*	$Header: /a/cvs/386BSD/ports/comm/flexfax/faxd/FaxServer.h,v 1.1 1993/08/31 23:42:32 ljo Exp $
/*
...
// server configuration
    ...
    int		deviceMode;		// protection mode for modem device
...
// FAX transmission protocol support
    void	sendFax(FaxRequest& fax,
		    const fxStr& number, const fxStr& canonicalNumber);
    fxBool	sendPrepareFax(FaxRequest& fax, fxStr& emsg);
    fxBool	sendClientCapabilitiesOK(u_int nsf, fxStr& emsg);
    fxBool	sendFaxPhaseB(FaxRequest& fax, const fxStr& file);
    void	sendPoll(FaxRequest& fax, fxBool remoteHasDoc);
    fxBool	sendSetupParams(TIFF*, Class2Params&, fxStr& emsg);
...
public:
    ...
    void sendFax(FaxRequest&);			// client-server send interface
