/*	$Header: /a/cvs/386BSD/ports/comm/flexfax/faxd/EverexSend.c++,v 1.1 1993/08/31 23:42:15 ljo Exp $
/*
...
EverexModem::sendTraining(Class2Params& params, fxStr& emsg)
{
	...
	do {
	    ...
	    char buf[1024];
	    if (getModemLine(buf, conf.t4Timer) > 2 && streq(buf, "R ", 2))
...
EverexModem::sendPPM(int ppm, fxStr& emsg)
{
	...
	if (getModemLine(rbuf, conf.t4Timer) > 2 && streq(rbuf, "R ", 2))
	    return (TRUE);
    } while (--t > 0);
