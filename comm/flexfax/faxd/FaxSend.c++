/*	$Header: /a/cvs/386BSD/ports/comm/flexfax/faxd/FaxSend.c++,v 1.1 1993/08/31 23:42:28 ljo Exp $
/*
 ...
 */
void
FaxServer::sendFax(FaxRequest& fax, const fxStr& number, const fxStr& canon)
{
	...
	"Unable to get remote capabilities or remote is not T.4 compatible",
	    ...
	    if (!sendClientCapabilitiesOK(nsf, notice)) {
     ...
     * the client's remote capabilities, update the machine
     * info database.
     */
    logTracingLevel = oTracingLevel;	// restore default value
    if (updateClientInfo)
	clientInfo.update(canon);
}
...
FaxServer::sendPoll(FaxRequest& fax, fxBool remoteHasDoc)
{
	...
	const char* cig = fax.files[i];
	if (*cig == '\0')
	    cig = FAXNumber;
	traceStatus(FAXTRACE_PROTOCOL, "POLL with CIG \"%s\"", cig);
...
FaxServer::sendFaxPhaseB(FaxRequest& fax, const fxStr& file)
{
	...
	if (sendSetupParams(tif, clientParams, fax.notice)) {
	    /*
 ...
 */
fxBool
FaxServer::sendClientCapabilitiesOK(u_int nsf, fxStr& emsg)
{
    /*
