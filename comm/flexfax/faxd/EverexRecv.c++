/*	$Header: /a/cvs/386BSD/ports/comm/flexfax/faxd/EverexRecv.c++,v 1.1 1993/08/31 23:42:14 ljo Exp $
/*
...
CallType
EverexModem::answerCall(AnswerType, fxStr& emsg)
{
...
CallType
EverexModem::answerResponse(fxStr&)
{
    return (CALLTYPE_FAX);	// XXX can't deduce, assume fax
}
...
EverexModem::recvTraining()
{
    ...
    fxBool ok = (!wasTimeout() && streq(buf, "M 3", 3));
    if (ok) {
...
EverexModem::recvPage(TIFF* tif, int& ppm, fxStr& emsg)
{
	...
	if (streq(rbuf, "M 1", 3)) {		// message carrier received
	    /*
	...
	} else if (streq(rbuf, "R ", 2)) {	// HDLC frames received
	    /*
