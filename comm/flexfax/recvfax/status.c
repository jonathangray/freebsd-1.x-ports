/*	$Header: /a/cvs/386BSD/ports/comm/flexfax/recvfax/status.c,v 1.1 1993/08/31 23:49:00 ljo Exp $
/*
...
sendClientJobStatus(Job* job, const char* jobname)
{
	...
	sendClient("jobStatus", "%s:%s:%s:%s:%s:%s",
	    jobname, job->sender, tts, job->number, job->modem, job->status);
    } else
	sendClient("jobStatus", "%s:%s:%d:%s",
	    jobname, job->sender, job->tts, job->number);
}
...
sendClientJobLocked(Job* job, const char* jobname)
{
	...
	sendClient("jobStatus", "%s:%s:locked:%s:%s:%s",
	    jobname, job->sender, job->number, job->modem, job->status);
    else
	sendClient("jobLocked", "%s:%s:0:%s",
	    jobname, job->sender, job->number);
}
