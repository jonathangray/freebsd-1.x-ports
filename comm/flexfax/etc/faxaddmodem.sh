...
LOCKDIR=/var/spool/lock	# UUCP locking directory
...
SERVERDIR=/usr/local/etc		# directory where servers are located
...
# server process.
# 
echo "Verifying your system is setup properly for fax service..."
...
#
x=`ypcat services 2>/dev/null | grep '^fax[ 	]'` 2>/dev/null
if [ -z "$x" -a -f $SERVICES ]; then
    x=`grep '^fax[ 	]' $SERVICES`
fi
if [ -z "$x" ]; then
    echo ""
    echo "There does not appear to be an entry for the fax service either in"
    echo "the yellow pages database or in the $SERVICES file;"
...
test -f $INETDCONF && {
    grep "fax[ 	]*stream[ 	]*tcp" $INETDCONF >/dev/null 2>&1 ||
    (echo ""
...
checkPort()
{
    if [ -f $CPATH.$1 -a -p $SPOOL/FIFO.$1 ]; then
	echo "There appears to be a modem already setup on $1,"
	...
	echo "Removing old FIFO special file $SPOOL/FIFO.$1."
	$QUIT $1 >/dev/null 2>&1; rm -f $SPOOL/FIFO.$1
	OLDCONFIG=$CPATH.$1
    fi
}
...
$QUIT $TTY >/dev/null 2>&1; sleep 1		# shutdown existing server
...
if [ -f $OLDCONFIG ]; then
    ...
    getParameter FAXNumber $OLDCONFIG;		FAXNumber="$param"
    getParameter DialStringRules $OLDCONFIG;	DialStringRules="$param"
    getParameter CountryCode $OLDCONFIG;	CountryCode="$param"
    getParameter AreaCode $OLDCONFIG;		AreaCode="$param"
    getParameter LongDistancePrefix $OLDCONFIG;	LongDistancePrefix="$param"
    getParameter InternationalPrefix $OLDCONFIG;InternationalPrefix="$param"
    getParameter ServerTracing $OLDCONFIG;	ServerTracing="$param"
    getParameter SessionTracing $OLDCONFIG;	SessionTracing="$param"
    getParameter RecvFileMode $OLDCONFIG;	RecvFileMode="$param"
    getParameter RingsBeforeAnswer $OLDCONFIG;	RingsBeforeAnswer="$param"
    getParameter SpeakerVolume $OLDCONFIG;	SpeakerVolume="$param"
    ...
    getParameter UseDialPrefix $OLDCONFIG;
    case "$param" in
    [yY]es|[oO]n)
	getParameter DialingPrefix $OLDCONFIG;	DialingPrefix="$param"
	;;
    esac
    getParameter GettyAllowed $OLDCONFIG;
    case "$GettyAllowed" in
    [yY]es|[oO]n)
	getParameter GettySpeed $OLDCONFIG;	GettyArgs="$param"
	;;
    esac
    getParameter QualifyTSI $OLDCONFIG;
    case "$QualifyTSI" in
    [yY]es|[oO]n)
	QualifyTSI="etc/tsi"
	;;
...
while [ "$ok" != "" -a "$ok" != "y" -a "$ok" != "yes" ]; do
    ...
    # make sure RecvFileMode is an octal number
    if [ "`expr "$RecvFileMode" : '\(.\)'`" != "0" ]; then
	RecvFileMode="0${RecvFileMode}"
    fi
    checkDialStringRules;
...
#
# Setup the sed commands for creating the configuration file:
#
ServerCmds="\
 -e '/FAXNumber:/s/+1.415.965.7824*/$FAXNumber/'\
 -e '/AreaCode:/s/415/$AreaCode/'\
 -e '/CountryCode:/s/1/$CountryCode/'\
 -e '/LongDistancePrefix:/s/1/$LongDistancePrefix/'\
 -e '/InternationalPrefix:/s/011/$InternationalPrefix/'\
 -e '/DialStringRules:/s;etc/dialrules;$DialStringRules;'\
 -e '/ServerTracing:/s/1/$ServerTracing/'\
 -e '/SessionTracing:/s/11/$SessionTracing/'\
 -e '/RecvFileMode:/s/0644/$RecvFileMode/'\
 -e '/SpeakerVolume:/s/off/$SpeakerVolume/'\
 -e '/RingsBeforeAnswer:/s/1/$RingsBeforeAnswer/'\
"
...
{
    protoMaxRate="$ModemMaxRate"
    protoRecvRate="$ModemRecvRate"
    protoXONXOFF="$ModemXONXOFF"
    ...
    protoHayesQueryCmd="$ModemHayesQueryCmd"
...
{
    getParameter ModemMaxRate $1;		ModemMaxRate=$param
    getParameter ModemRecvRate $1;		ModemRecvRate=$param
    getParameter ModemXONXOFF $1;		ModemXONXOFF=$param
    ...
    getParameter ModemHayesQueryCmd $1;		ModemHayesQueryCmd=$param
...
{
    ModemCmds=""
    addSedCmd "$protoMaxRate"		"$ModemMaxRate"		MaxRate
    addSedCmd "$protoRecvRate"		"$ModemRecvRate"	RecvRate
    ...
    addSedCmd "$protoXONXOFF"		"$ModemXONXOFF"		XONXOFF
    ...
    addSedCmd "$protoHayesQueryCmd"	"$ModemHayesQueryCmd"	HayesQueryCmd
...
checkFlowControlAgainstTTY()
{
	...
	if [ "$TTY" = "ttyf${PORT}" -a -c /dev/ttym${PORT} ]; then
	    ...
	    read x
	    if [ -z "$x" -o "$x" = "y" -o "$x" = "yes" ]; then
		TTY="ttym${PORT}"; CONFIG=$CPATH.$TTY
	    fi
	fi
	;;
    [oO]ff|[nN]o)
	if [ "$TTY" = "ttym${PORT}" -a -c /dev/ttyf${PORT} ]; then
	    ...
	    read x
	    if [ -z "$x" -o "$x" = "y" -o "$x" = "yes" ]; then
		TTY="ttyf${PORT}"; CONFIG=$CPATH.$TTY
	    fi
	fi
	;;
    esac
...
printModemConfig()
{
    ...
    echo ""
    printIfNotNull "ModemMaxRate:	"	"$ModemMaxRate"
    printIfNotNull "ModemRecvRate:	"	"$ModemRecvRate"
    printIfNotNull "ModemSendFillOrder:"	"$ModemSendFillOrder"
    printIfNotNull "ModemRecvFillOrder:"	"$ModemRecvFillOrder"
    printIfNotNull "ModemXONXOFF:	"	"$ModemXONXOFF"
    ...
    printIfNotNull "ModemHayesQueryCmd:"	"$ModemHayesQueryCmd"
...
{
    test "$protoMaxRate" && { 
	promptForNumericParameter "$ModemMaxRate" \
	    "Maximum rate for DCE-DTE communication";
	ModemMaxRate=$param;
    }
    test "$protoRecvRate" && {
	promptForNumericParameter "$ModemRecvRate" \
	    "Fixed rate for receiving facsimile";
	ModemRecvRate=$param;
    }
    test "$protoXONXOFF" && {
	promptForBooleanParameter "$ModemXONXOFF" \
	    "Use XON/XOFF flow control between host and modem"; \
	ModemXONXOFF=$param;
    }
    ...
    test "$protoHayesQueryCmd" && {
	promptForATCmdParameter "$ModemHayesQueryCmd" \
	    "Command to do a Hayes-style capabilities query";
	ModemHayesQueryCmd=$param;
    }
...
chown $faxUID $CONFIG; chgrp $faxGID $CONFIG; chmod 644 $CONFIG
...
chown $faxUID $FIFO; chgrp $faxGID $FIFO; chmod 600 $FIFO
