/*	$Header: /a/cvs/386BSD/ports/comm/flexfax/util/SendFaxClient.c++,v 1.1 1993/08/31 23:50:05 ljo Exp $
/*
...
SendFaxClient::~SendFaxClient()
{
    for (u_int i = 0; i < coverPages.length(); i++)
...
SendFaxClient::prepareSubmission()
{
	...
	if (!handleFile((*files)[i]))
	    return (FALSE);
    // suppress cover page if we're just doing a poll
    if (pollCmd && files->length() == 0)
	coverSheet = FALSE;
    if (coverSheet) {
	coverPages.resize(destNumbers.length());
	for (u_int i = 0; i < destNumbers.length(); i++)
	    coverPages[i] = makeCoverPage(destNames[i], destNumbers[i],
...
)
{
	...
	cmd.append(" -f \"" | sender | "\"");
	if (getCoverComments() != "")
	    cmd.append(" -c \"" | getCoverComments() | "\"");
	if (getCoverRegarding() != "")
	    cmd.append(" -r \"" | getCoverRegarding() | "\"");
	if (location != "")
	    cmd.append(" -l \"" | location | "\"");
	if (company != "")
	    cmd.append(" -x \"" | company | "\"");
	if (getTotalPages() > 0)
	    cmd.append(" -p " | fxStr((int) getTotalPages(), "%u"));
	if (getVerbose())
