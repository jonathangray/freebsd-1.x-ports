...
The most recent version is V2.1; it is implemented by Version 2.1 of the
	Fig graphics editor, which runs under SunView.

Previous versions of Fig code which have been widely used are \TFX,
	and V2.0.
\TFX, which was defined by the TransFig package, was a compatible extension to
	the Fig code V1.4, as defined by the last official version of Fig.
V2.0 was implemented by XFig, 
	which runs under Version 11 of the X Windowing System.
The definition of V2.0 was incompatible with \TFX, and had features
	which made translation into output forms other than PostScript
	difficult.
XFig also supports \TFX compatibility as a compile-time option.

TransFig supports \TFX, and supports translation of V2.0 to PostScript.
In order to unify the various versions of Fig code, V2.1 supports
	the PS-specific features of V2.0 {\em as an option}.
Thus, V2.1 can describe figure in a manner which is either highly
	portable, as with \TFX, or in a way which makes use of PostScript's
	powerful features.
A future release of XFig may implement V2.1.
...
The values between {\tt WHITE\_FILL} and {\tt BLACK\_FILL} define a gray scale;
...
\hline
%
{\tt RIGID\_TEXT}	& 001 	&  Font size scales with magnification\\ \hline
%
{\tt SPECIAL\_TEXT}	& 010	&  Text includes formatting commands \\ \hline
%
{\tt PSFONT\_TEXT}	& 100	&  Font field Specifies PS font	\\ \hline
\end{tabular}
\end{center}

The {\tt RIGID\_TEXT} flag is used to preserve the size of text objects
	relative to graphical objects as the figure is scaled.
...
Finally, the {\tt PSFONT\_TEXT} flag changes the interpretation of the font
	field.
