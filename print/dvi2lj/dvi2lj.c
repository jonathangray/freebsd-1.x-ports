...
#define  INT_ASCII(b,i) if (i == 0) b[0] = '\0'; else sprintf(b,"%hd",i)
#endif
/**********************************************************************/
            ...
            if (DoublePage && !SkipMode) {
			   ...
			   if (PrintFirstPart) {
			     ...
			     FormFeed();
                  ...
                  WouldPrint ++;
/*
    printf("doublepage %d, Reverse %d, WouldPrint %d, fpZ %d\n",
        (int)DoublePage, (int)Reverse, (int)WouldPrint, (int)ZeroPage);
*/
		}
		 ...
		 SkipMode = (bool)(PageParity != (short)(count[0]%2));
...
#ifdef TIMING
        ftime(&timebuffer);
        time = ((timebuffer.time) + (timebuffer.millitm) / 1000.0) -start_time;
...
char    *argv[];
{
    ...
    while (argind < argc) {
		...
		if (*tcp >= '0' && *tcp <= '9') {
		  int tmp;
		  sscanf(tcp, "%hd", &tmp);
		  if (tmp % 2 == 0) PrintFirstPart = FALSE;
...
#endif
            case '-':
		system ("cat > /tmp/dvi.lj");
		dvifp = BINOPEN("/tmp/dvi.lj");
                break;
            default:
...
*/
    ...
    if (!(font_found = (bool)
          findfile(PXLpath, tfontptr->n, tfontptr->font_mag,
          tfontptr->name))) {
    ...
    Warning("font = \"%s\",\n->tfm checksum = %lX,\n->pxl checksum = %lX",
        ...
        for (i = FIRSTFNTCHAR; i <= LASTFNTCHAR; i++) {
            ...
            if (tcharptr->yOffset > 0)
                max(tfontptr->max_yoff,tcharptr->yOffset);
...
#endif
/*
fprintf(stderr,"char=%d, tcharptr=%lx, flag_byte=%d, font=%lx\n",car, tcharptr,flag_byte,tfontptr);
*/
