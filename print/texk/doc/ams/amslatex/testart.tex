...
%%   It is a sample document illustrating the use of the ``amsart''
%%   documentstyle and the ``amstex'' option."
%%}
%%% end of file header
%
\documentstyle[draft%
...
%      A method of making the syntax check decision interactively. To
%      see how this works, remove the double %% from the following
%      lines and also from the `syntonly' option above, in the
%      documentstyle options list. \let and \csname are primitive TeX
%      commands documented in the TeXbook, but not in the LaTeX manual.
%
...
%       Some definitions useful in producing this sort of
%       documentation:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%     This combination of author names, addresses, and e-mail
%%     addresses is carefully selected to illustrate most of the
%%     features of the commands involved. Real-life author/address
%%     combinations will rarely be this complicated!
%% First author
\author{David Belar}
\address[D. Belar and L. Eifrig]{Department of Electrical Engineering\\
        University of Minnesota\\ Minneapolis, Minnesota 55455}
%% Note the doubled @@:
\email[D.~Belar]{davidb@@egr.umn.edu}
...
\curraddr[L. Eifrig]{San Diego Supercomputer Center\\P.O. Box 85608\\
  San Diego, California 92138}
%% Note the doubled @@:
\email[L.~Eifrig]{eifrig@@sds.sdcs.edu}
...
\address[S. N\"a\"at\"anen]{MIT Laboratory for Computer Science \\
...
\begin{abstract}
This paper is a sample illustrating  the use of the \opt{amstex}
option in \LaTeX{}, and the American Mathematical Society preprint
documentstyle, \opt{amsart}.
...
of extracts from published papers, interspersed with sample \TeX{}
coding, instructions to authors, and comments about the use of
particular commands.

Sections \ref{s:font} and ~\ref{s:comp} are devoted to examples
of the commands described in the \AmS-\LaTeX\ users guide.
...
and \verb=\maketitle=.%
%
\footnote{Actually, as stated in the \LaTeX{} manual, the top matter
...
the \LaTeX{} manual, but it has an additional option like sectioning
...
no abbreviations are used in addresses.  Indicate the
normal line breaks that would be made on a mailing
label using \verb=\\=; in the current version of the
{\tt amsart} documentstyle, these \verb=\\='s will be
replaced with commas, since author addresses are set
in paragraph form, but other documentstyles or
a future version of the {\tt amsart} documentstyle
might treat the information differently.
...
If the current address of an author is no longer the institution where
the research being described took place, the current address can be
provided through the \verb=\curraddr= command.  Electronic mail
addresses should also be given, when applicable, using \verb=\email=.

For multiple authors with assorted current addresses and e-mail
addresses, square-bracket options can be used to indicate those
individuals to which an address, current address, or e-mail address
pertains. For example, for two authors at the same institution, with
different e-mail addresses, you would enter the information as
\begin{verbatim}
%      Address of A. Author and B. Colleague:
\address{North Carolina State University\\Raleigh,
North Carolina 27695}
\email[A. Author]{auth@@abel.ncsu.edu}
\email[B. Colleague]{bcolleague@@fourier.ncsu.edu}
\end{verbatim}
...
\end{equation*}
$|\widetilde{D}u|$-almost everywhere in $\bold R$. It is well known
...
everywhere and $\lim_{s\to t^-}\hat w(s)=\hat w(t)$ for every $t\in\bold
...
%% Added \normalshape here to avoid error message about
%% cmtcsc10 font not found (for those who don't happen to
%% have it).
...
\subsection{\cs{overset}, \cs{underset}, and \cs{sideset}} 
Examples:
...
\subsection{The \cs{text} command}
...
\opt{amstex} option:
\cs{varlimsup}, \cs{varliminf}, \cs{varinjlim}, 
and \cs{varprojlim}.
...
\subsection{\cs{mod} and its relatives} 
...
\cs{hdotsfor}\verb"{"{\it number\/}\verb"}" produces a row of dots in a matrix
...
\noindent{\large\bf Note: Starting on the following page, vertical rules are
...
If there is not enough room for it, the equation number for  a {\tt
split} will be shifted to the previous line, when equation numbers are
...
\end{align*}
\end{gather}
Here the {\tt split} environment gets a number from the outer {\tt
gather} environment; numbers for individual lines of the {\tt align*}
...
The \verb|*|-ed form of {\tt gather} with the non-\verb|*|-ed form of
{\tt align}.
\begin{gather*}
\begin{split} \varphi(x,z)
