#define VERSION "dvilj version 1.1"
/**********************************************************************
 ****************************  Intro  *********************************
 **********************************************************************
...
char    *argv[];
{
    ...
    {
      /* Kpathsea initialization. */
      char *makepk = getenv ("DVILJMAKEPK");
      /* Might have a program-specific name for MakeTeXPK itself.  */
      if (makepk)
        KPSE_MAKE_SPEC_PROGRAM (kpse_make_specs[kpse_pk_format])
          = makepk;
      KPSE_MAKE_SPEC_ENABLED (kpse_make_specs[kpse_pk_format])
        = makeTexPK;

      xputenv_int ("MAKETEX_BASE_DPI", (int) (RESOLUTION + .5));

      kpse_fallback_font = "cmr10";
      kpse_init_fallback_resolutions ("DVILJSIZES");
      kpse_font_override_path = getenv ("DVILJFONTS");
...
*/
    ...
    {
      kpse_font_file_type font_ret;
      char *name;
      unsigned dpi = tfontptr->font_mag / 5.0 + .5;
      
      if (makeTexPK)
        kpse_set_maketex_mag (dpi, RESOLUTION);
      ...
      if (name)
        {
          ...
          else if (!kpse_check_bitmap_tolerance ((double) font_ret.dpi,
                                                 (double) dpi))
            fprintf (stderr,
