...
#endif
   {
     char *name = concat (d, n);

     xputenv ("MAKETEX_DPI", itoa (fd->dpi));
     kpse_set_maketex_mag (fd->dpi, DPI);

     /* Don't run MakeTeXPK if it's been globally disabled or
        if the font is absolute_p.  */
     KPSE_MAKE_SPEC_ENABLED (kpse_make_specs[kpse_pk_format])
       = !dontmakefont && !*d;

     pkfile=pksearch(pkpath, name, READBIN, fd->dpi, &name_ret, &dpi_ret);

     if (!pkfile || !STREQ (name, name_ret))
       {
         (void)sprintf(errbuf,
     ...
     else if (!kpse_check_bitmap_tolerance ((double) dpi_ret,
                                            (double) fd->dpi))
       {
