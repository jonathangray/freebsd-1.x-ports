 ...
 *      SELFILE use the SelFile selection widget
 */
...
char *dvi_oops_str = NULL;
#define	dvi_oops(str) \
  do { dvi_oops_str = str; longjmp(dvi_env, 1); } while (0)
	...
	struct font *fontp;
{
	...
	int	magic;
        ...
        if (use_maketexpk)
          /* In case font_open winds up calling MakeTeXPK.  */
          kpse_set_maketex_mag (dpi, pixels_per_inch);

	fontp->flags |= FONT_LOADED;
	fontp->file = font_open(fontp->fontname, &font_found,
	    (WIDEARG(float, double)) fsize, &size_found, 
	    &fontp->filename);
	if (fontp->file == NULL) {
	    Fprintf(stderr, "xdvi: Font %s not found.\n", fontp->fontname);
	    return True;
	}
	--n_files_left;
	if (font_found != NULL) {
	    Fprintf(stderr,
		    "xdvi: Font %s not found, using %s at %d instead.\n",
		    fontp->fontname, font_found, dpi);
	    free(fontp->fontname);
	    /* We free this memory, so malloc it.  */
	    fontp->fontname = xstrdup (font_found);
	}
	else if (!kpse_check_bitmap_tolerance ((double) size_found / 5.0,
                                               (double) dpi))
          Fprintf(stderr, "xdvi: Font %s at %d not found, using %d instead.\n",
...
#if 0
/* We don't use this in xdvik any more.  */
...
/* xdvik doesn't use this any more; the magstep-correction code
   is in kpathsea, where it can be shared with other drivers.  But keep the
   variable around for old-times' sake.  */
#if 0
	...
	for (fontp = font_head;; fontp = fontp->next) {
		...
		if (list_fonts)
		    Printf("xdvi: Looking for font %s at %d dpi\n", fontname, (int) (fsize + 0.5));
...
open_dvi_file()
{
  char *extension = strrchr (dvi_name, '.');
  int err = setjmp (dvi_env);
  
  if (err) oops(dvi_oops_str);

  /* Prefer `foo.bar.dvi' to `foo.bar' -- add .dvi unless that is
     already the extension.  */
  ...
  if (!init_dvi_file ())
    {
      fprintf (stderr, "init_dvi_file (%s) failed.\n", dvi_name);
      exit (1);
    }
}
...
check_dvi_file()
{
	...
	if (dvi_file == NULL || fstat(fileno(dvi_file), &fstatbuf) != 0
		...
		for (fontp = font_head; fontp != NULL; fontp = fontp->next)
		    fontp->flags &= ~FONT_IN_USE;
#ifdef SELFILE
		if (dvi_time > fstatbuf.st_mtime) /* choose a new file time */
