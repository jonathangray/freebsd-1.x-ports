 ...
 *	SELFILE support for the SelFile selection widget
 */
...
/* We use `string' as a variable below, but kpathsea makes it a type.  */
#define string string_var
...
static	struct {_Xconst char	a[50], b, c, d, e, f;}

#ifndef X10
	version = {VERSION_STRING, 0, 0, 0};
#else
	version = {"This is xdvi for X10, patchlevel ", 0,
		0, 0};
#endif
	...
	Boolean	warn;
{
	...
	if (!colors_allocated)
	{
	    ...
	    if (RESOURCE(xd_gamma) == 0.0) RESOURCE(xd_gamma) = 1.0;
	    ...
	    for (i = 0; i < 16; ++i) {
		double	pow();
		double	frac = RESOURCE(xd_gamma) > 0 ?
		    pow((double) i / 15, 1 / RESOURCE(xd_gamma))
		    : 1 - pow((double) (15 - i) / 15, -RESOURCE(xd_gamma));
...
/*
 *	Put a grey (dithered) on the screen.  This is called by epsfile.
 */

void
put_grey_rectangle(x, y, w, h)
  int x, y, w, h;
  /* No ifndef X10 here.  Surely no one is still runing X10.?.? */
{
  static Boolean first_call = True;
  static GC greyGC;
  static Pixmap greypm;
  static unsigned char pmdata[] = {0x01, 0x02};

  if (first_call) {
    greyGC = XCreateGC(DISP, RootWindowOfScreen(SCRN), 0, NULL);
    greypm = XCreateBitmapFromData(DISP, RootWindowOfScreen(SCRN),
				   (char*) pmdata, 2, 2);
    XSetForeground(DISP, greyGC, RESOURCE(fore_Pixel));
    XSetBackground(DISP, greyGC, RESOURCE(back_Pixel));
    XSetStipple   (DISP, greyGC, greypm);
    XSetFillStyle (DISP, greyGC, FillStippled);
    first_call = False;
  }

  if (x < max_x && x + w >= min_x && y < max_y && y + h >= min_y) {
    if (--event_counter == 0) read_events(False);
    XFillRectangle(DISP, WINDOW(currwin), greyGC,
		   x - currwin.base_x, y - currwin.base_y, w, h);
  }
}
...
#ifdef SELFILE
            case 'F':
		++dvi_time ;    /* then it wil notice we want a new file  (hack!) JSP */
		break ;
#endif  /* SELFILE */
	    case 'R':
		/* reread DVI file */
...
/* remove .dvi from a name by zapping the dot to a zero, done the tedious way JSP */
void
rmdotdvi P1C(char *, name)
{
  int i ;
  i = strlen(name) ;
  if ((i > 4) && (name[i-4] == '.') &&
      (name[i-3] == 'd') && (name[i-2] == 'v') &&
      (name[i-1] == 'i'))
    name[i-4] = 0 ;
}
...
/* Allow the user to choose a new dvi file, by popping up a dialog box
   ...
   maybe we should only allow files ending in .dvi to be selected.   JSP
*/
FILE *select_filename(open, move_home)
int open, move_home ;
{
...
/* we may not want the file they returned... */
    if (!open)
      fclose (dummy_file) ;
    /* the name is what we really want, so use it  JSP */
    dvi_name = dummy_name ;
    current_page = 0 ;  /* go to start of new dvi file */
    if (move_home)
      home(False);      /* Move to home position on new first page */
#ifdef TOOLKIT
    /* Attempt to set the name correctly (only try for toolkit
       version) */

    {
      /* Use basename of DVI file for name in icon/title  */
      char *dummy_name = strrchr (dvi_name, '/');

      dummy_name = dummy_name ? dummy_name + 1 : dvi_name;
      Sprintf(title_name, "Xdvi: %s", dummy_name);
      rmdotdvi(title_name) ;
      Sprintf(icon_name, "%s", dummy_name);
      rmdotdvi(icon_name) ;
/* Set the name of the icon/title here */
      temp_args[0].value = (XtArgVal) title_name;
      temp_args[1].value = (XtArgVal) icon_name;
      XtSetValues(top_level, temp_args, XtNumber(temp_args));
    }
/*
    Sprintf(title_name, "Xdvi: %s", dvi_name);
    temp_args[0].value = (XtArgVal) title_name;
    XtSetValues(top_level, temp_args, XtNumber(temp_args));
*/
#endif  /* TOOLKIT */
	...
	struct WindowRec *windowrec;
{
	...
	Flush();
	if (setjmp(dvi_env)) {
	    extern char *dvi_oops_str;
	    char *errtext = dvi_oops_str;
	    ClearPage(mane);
#ifndef X10
	    get_xy();
...
#ifdef SA_RESTART
	/* The EPSF handling and MakeTeXPKfails on the Alpha without
	   this, because SIGIO interrupts the call of system(3).  (OSF/1
...
{"-noepsf",     ".noEPSF",        XrmoptionNoArg,       (caddr_t) "on"},
{"+noepsf",     ".noEPSF",        XrmoptionNoArg,       (caddr_t) "off"},
{"-epsfgrey",   ".EPSFGrey",      XrmoptionNoArg,       (caddr_t) "on"},
{"+epsfgrey",   ".EPSFGrey",      XrmoptionNoArg,       (caddr_t) "off"},
...
{"noEPSF", "noEPSF", XtRBoolean, sizeof(Boolean),
     offset(no_epsf), XtRString, "false"},
{"EPSFGrey", "EPSFGrey", XtRBoolean, sizeof(Boolean),
     offset(epsf_grey), XtRString, "false"},
...
{"maketexpk", "MakeTeXPK", XtRBoolean, sizeof (Boolean),
 offset(use_maketexpk), XtRString, 
#ifdef MAKETEXPK
 "true"
#else
 "false"
...
#ifdef	GREY
{"-gamma",	"gamma",	SepArg,	FloatArg, 1,	(caddr_t) &xd_gamma},
#endif
...
{"-altfont",	"altFont",	SepArg,	StringArg, 1,	(caddr_t) &kpse_fallback_font},
...
{"-noepsf",     NULL,        TrueArg,  BooleanArg, 2, (caddr_t) &no_epsf},
{"+noepsf",     "noEPSF",    FalseArg, BooleanArg, 1, (caddr_t) &no_epsf},
{"-epsfgrey",   NULL,        TrueArg,  BooleanArg, 2, (caddr_t) &epsf_grey},
{"+epsfgrey",   "EPSFGrey",  FalseArg, BooleanArg, 1, (caddr_t) &epsf_grey},
...
#endif
{"-maketexpk",	NULL,       FalseArg, BooleanArg, 2, (caddr_t) &use_maketexpk},
{"+maketexpk",	"maketexpk", TrueArg, BooleanArg, 1, (caddr_t) &use_maketexpk},
{"-mfmode",	"mfmode",       SepArg, StringArg, 1, (caddr_t) &mf_mode},
};
	...
	char **argv;
{
	...
	title_name = xmalloc(BUFSIZ, "xdvi title");
	icon_name  = xmalloc(BUFSIZ, "xdvi icon");
	...
	no_epsf = resource.no_epsf;
	epsf_grey = resource.epsf_grey;
	use_maketexpk = resource.use_maketexpk;
	mf_mode = resource.mf_mode;
...
#ifdef TOOLKIT
        /* If can't find right font, may as well try cmr10.  Do this
           here instead of init_font_open so we don't have to import the
           resource structure.  */
        kpse_fallback_font = RESOURCE (alt_font);
#else
	if (!kpse_fallback_font)
	  kpse_fallback_font = ALTFONT;
#endif
	
	/* Program-specific fallback resolutions? */
        kpse_init_fallback_resolutions ("XDVISIZES");

        /* Might have a program-specific font path.  */
        kpse_font_override_path = getenv ("XDVIFONTS");

	/* For MakeTeXPK. Maybe we don't need to do any of this stuff if
	   use_maketexpk is false? But seems better to just always do
	   it, in case it can change dynamically. */
	{
	  char *makepk = getenv ("XDVIMAKEPK");
          /* Might have a program-specific name for MakeTeXPK itself.  */
          if (makepk)
            KPSE_MAKE_SPEC_PROGRAM (kpse_make_specs[kpse_any_glyph_format])
              = makepk;
          KPSE_MAKE_SPEC_ENABLED (kpse_make_specs[kpse_any_glyph_format])
            = use_maketexpk;

          xputenv_int ("MAKETEX_BASE_DPI", (int) (pixels_per_inch + .5));
          
          /* If no cmdline or resource, try envvar, then compile-time. */
#ifndef MFMODE
#define MFMODE "cx"
#endif
	  if (!mf_mode)
	    mf_mode = getenv ("MAKETEX_MODE");
	  if (!mf_mode)
	    mf_mode = MFMODE;
          xputenv ("MAKETEX_MODE", mf_mode);
        }
...
#define	xtra_wid	0
...

	{
	    ...
	    rmdotdvi(icon_name) ;
            ...
            XSetStandardProperties (DISP, XtWindow (top_level), title_name,
                                    icon_name, None,
                                    argv, argc, NULL);
	    XGetWindowAttributes(DISP, WINDOW(mane), &attrs);
	    backing_store = attrs.backing_store;
