 ...
 * 
 * $NCDId: @(#)ausgi.c,v 1.3 1994/01/21 22:55:17 greg Exp $
 * 
...
writePhysicalOutputsMono()
{
    ...
    if (we_are_16 ) {
      AuNativeToUnsigned16LSB(auOutputMono, 1, auMinibufSamples);    
    } else {
      AuNativeToUnsigned8(auOutputMono, 1, auMinibufSamples);    
    }


    write(audiofd, auOutputMono, auMinibufSamples);
...
{    
  ...
  int dsp_stereo;
    ...
    if (we_are_16 ) {
      AuNativeToUnsigned16LSB(auOutputStereo, 1, auMinibufSamples);    
    } else {
      AuNativeToUnsigned8(auOutputStereo, 1, auMinibufSamples);    
    }
...
/*    write(audiofd, auOutputStereo, auMinibufSamples);  */

    fprintf(stderr, "write to card stereo \n");
    write(audiofd, auOutputStereo, 2 * auMinibufSamples);
