 ...
 * 
 * $NCDId: @(#)auutil.c,v 1.10 1994/01/21 22:54:19 greg Exp $
 */
...
AuBool          discard;
{
    ...
    if (lowWater)		/* must be an import */
	port->lastRead = port->minibuf + PAD4(minibufSize);
    else			/* an export */
	port->lastWrite = port->minibuf + PAD4(minibufSize);
                ...
                numTracks;
{
    ...
    c->lastWrite = c->minibuf + PAD4(minibufSize);
    c->write = c->lastWrite + PAD4(minibufSize) * 2;
...
int             form;
{
    ...
    c->refCnt = 0;
    c->minibuf = (AuUint8 *) c + PAD4(sizeof(ComponentRec));
    c->lastRead = c->minibuf + PAD4(minibufSize);
    c->minibufSize = minibufSize;
    c->physicalDeviceMask = NotAPhysicalDevice;
    c->kind = AuComponentKindOther;
	...
	case AuWaveFormSquare:
	    c->read = (AuUint8 *) squareWave;
	    c->waveSize = sizeof(squareWave);
	    break;
	case AuWaveFormSine:
	    c->read = (AuUint8 *) sineWave;
	    c->waveSize = sizeof(sineWave);
	    break;
    }
	...
	case AuElementTypeImportWaveForm:
		...
		if (el->type == AuElementTypeImportBucket)
		{
		    if (!(input->flowEl->minibuf = (AuUint8 *)
			  aualloc(PAD4(input->component->minibufSize) +
				  input->component->numTracks *
				  sizeofFormat(input->component->format))))
			AU_ERROR(AuBadAlloc, 0);

		    input->flowEl->lastRead = input->flowEl->minibuf +
			PAD4(input->component->minibufSize);
		    input->component->refCnt++;
		}
		else
	...
	switch (el->type)
	{
		...
		if (!recompile)
		{
			...
			if (!(output->flowEl->minibuf = (AuUint8 *)
			      aualloc(PAD4(output->component->minibufSize) +
				  sizeofFormat(output->component->format))))
			    AU_ERROR(AuBadAlloc, 0);

			output->flowEl->lastWrite = output->flowEl->minibuf +
			    PAD4(output->component->minibufSize);
			output->component->refCnt++;
		    }
		    else
...
int             reason;
{
	...
	if (el->compiled &&
		...
		case AuElementTypeImportWaveForm:
		    {
			int             scale = el->component->waveSize /
			auNativeBytesPerSample;

			el->component->wavePhase = el->component->readPhase = 0;
			el->countSamples = el->raw->importwaveform.num_samples !=
			    AuUnlimitedSamples;

			if (el->countSamples)
			    el->numBytes = el->raw->importwaveform.num_samples *
				sizeofFormat(el->component->format) *
				el->component->numTracks * scale;

			el->component->frequency =
			    el->raw->importwaveform.frequency * scale;
			break;
		    }
		case AuElementTypeImportDevice:
		...
		case AuElementTypeImportBucket:
			...
			el->readPhase = 0;
			n = sizeofFormat(el->component->format) *
			    el->component->numTracks;
		...
		case AuElementTypeExportMonitor:
			...
			el->component->writePhase = 0;
			p = (AuUint32 *) el->component->lastWrite;

			for (i = 0; i < (int) el->component->numTracks; i++)
			    *p++ = 0x7fff8001;
			break;
		    }
	    }
...
ComponentPtr    c;
{
	...
	if (mask & 1)
		...
		case AuCompCommonDescriptionMask:
		    ...
		    if (strncmp(c->description.string, varData,
