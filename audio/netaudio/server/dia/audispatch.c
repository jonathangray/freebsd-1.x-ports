 ...
 * 
 * $NCDId: @(#)audispatch.c,v 1.8 1994/02/23 23:18:56 greg Exp $
 */
...
extern FlowPtr  auClockedFlows,
                auUnclockedFlows;
extern unsigned int auBytesPerSample[];
...
ClientPtr       client;
{
	...
	c = el->component;
	d = c->data + el->raw->exportbucket.offset *
	    sizeofFormat(c->format) * c->numTracks + flow->count;
...
ClientPtr       client;
{
	...
	el = &flow->elements[stuff->element_num ? 0 : 1];
	c = el->component;
	s = c->data + el->raw->importbucket.offset *
	    sizeofFormat(c->format) * c->numTracks + flow->count;
...
AuInitSetupReply()
{
    ...
    auSetupPrefix.length = (sizeof(auConnSetup) + len) >> 2;
