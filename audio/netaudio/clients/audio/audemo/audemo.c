 ...
 *
 * $NCDId: @(#)audemo.c,v 1.51 1993/11/03 19:23:18 greg Exp $
 */
...
static int      ElementCount,
                MonitorCount;
						 ...
						 * this? */
#define strdup ncd_strdup	/* To avoid conflicting with headers on
				   hosts that *do* have strdup... */
char           *
strdup(s)
char           *s;
{
...
static void
meterToggleCB(w, globals, call_data)
Widget          w;
GlobalDataPtr   globals;
XtPointer       call_data;
{
...
static void
newBucketCB(w, globals, call_data)
Widget          w;
GlobalDataPtr   globals;
XtPointer       call_data;
{
...
static void
bucketPlayCB(w, globals, call_data)
Widget          w;
GlobalDataPtr   globals;
XtPointer       call_data;
{
    ...
    if (AuSoundPlayFromBucket(globals->aud,
			      AuBucketIdentifier(&buf->list[index]), AuNone,
...
static void
bucketQueryCB(w, globals, call_data)
Widget          w;
GlobalDataPtr   globals;
XtPointer       call_data;
{
...
static void
bucketDeleteCB(w, globals, call_data)
Widget          w;
GlobalDataPtr   globals;
XtPointer       call_data;
{
    ...
    AuDestroyBucket(globals->aud, AuBucketIdentifier(&buf->list[index]), NULL);
    bucketQueryCB(buf->query, globals, NULL);
}

static void
bucketLoadCB(w, globals, call_data)
Widget          w;
GlobalDataPtr   globals;
XtPointer       call_data;
{
    ...
    AuSoundCreateBucketFromFile(globals->aud, globals->fileNames[index],
				AuAccessAllMasks, NULL, NULL);
    bucketQueryCB(globals->buf.query, globals, NULL);
}

static void
bucketsCB(w, globals, call_data)
Widget          w;
GlobalDataPtr   globals;
XtPointer       call_data;
{
...
static void
bucketRecordStartCB(w, globals, call_data)
Widget          w;
GlobalDataPtr   globals;
XtPointer       call_data;
{
    ...
    if (AuSoundRecordToBucket(globals->aud,
			      AuBucketIdentifier(&buf->list[index]),
...
static void
recordStartCB(w, globals, call_data)
Widget          w;
GlobalDataPtr   globals;
XtPointer       call_data;
{
...
static void
bucketSaveCB(w, globals, call_data)
Widget          w;
GlobalDataPtr   globals;
XtPointer       call_data;
{
...
static void
bucketDismissCB(w, globals, call_data)
Widget          w;
GlobalDataPtr   globals;
XtPointer       call_data;
{
...
static void
recordDismissCB(w, globals, call_data)
Widget          w;
GlobalDataPtr   globals;
XtPointer       call_data;
{
...
static void
bucketRecordDismissCB(w, globals, call_data)
Widget          w;
GlobalDataPtr   globals;
XtPointer       call_data;
{
...
static void
bucketRecordCB(w, globals, call_data)
Widget          w;
GlobalDataPtr   globals;
XtPointer       call_data;
{
...
DonePrivPtr     data;
{
    ...
    if (AuSoundPlayFromFile(globals->aud, globals->fileNames[index], AuNone,
...
static void
bucketListCB(w, globals, listInfo)
Widget          w;
GlobalDataPtr   globals;
...
static void
scrollProcCB(w, globals, position)
Widget          w;
GlobalDataPtr   globals;
int             position;
{
...
static void
gainScrollCB(w, globals, position)
Widget          w;
GlobalDataPtr   globals;
int             position;
{
...
char           *dir;
{
    ...
    sprintf(buf, "NCDaudio %d.%d", AuServerProtocolMajorVersion(g->aud),
...
GlobalDataPtr   g;
{
    ...
    maxX = x + width - 2;	/* the -2 is a hack since "version" has no
				 * border */
