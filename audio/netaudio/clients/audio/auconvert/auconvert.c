 ...
 *
 * $NCDId: @(#)auconvert.c,v 1.6 1994/01/10 20:05:43 greg Exp $
 */
...
char          **argv;
{
    ...
    if (rawFormat == -1)
    {
	if (SoundReadFile(data, SoundNumBytes(in), in) != SoundNumBytes(in))
    ...
    out = SoundCreate(fileFormat == -1 ? SoundFileFormat(in) : fileFormat,
		      outputDataFormat, SoundNumTracks(in),
    ...
    if (!SoundValidateDataFormat(out))
    {
	fprintf(stderr, "%s file format does not support %s data.\n",
		SoundFileFormatToString(SoundFileFormat(out)),
		AuFormatToString(SoundDataFormat(out)));
	exit(1);
    }
    ...
    {
	if (SoundWriteFile(data, numBytes, out) != numBytes)
	    fatalError("Error writing output file");
    }
    else
	rateConvert(in, out, numBytes, data);
