 ...
 *
 * $NCDId: @(#)Graph.c,v 1.9 1993/11/03 19:23:23 greg Exp $
 */
                ...
                w;
{
    ...
    {
	XtCallCallbacks(w, XtNleftProc, (XtPointer) w->graph.leftMarker);
	redraw = TRUE;
    }

    if (w->graph.rightMarker != old->graph.rightMarker)
    {
	XtCallCallbacks(w, XtNrightProc, (XtPointer) w->graph.rightMarker);
	redraw = TRUE;
    }
