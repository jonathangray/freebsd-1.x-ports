 ...
 *
 * $NCDId: @(#)auedit.c,v 1.28 1994/02/10 18:25:47 greg Exp $
 */
...
XtRequestId    *id;
{
    ...
    if (XFindContext(g->dpy, *id, g->selection.context, &con))
    {					       /* first call */
    ...
    {
	free(con);
	XDeleteContext(g->dpy, *id, g->selection.context);
	...
	*length = 0;
    ...
    con->len -= len;
    con->p += len;
    XSaveContext(g->dpy, *id, g->selection.context, con);
...
static void
selectionReceive(w, g, selection, type, value, length, format)
Widget          w;
GlobalDataPtr   g;
Atom           *selection;
Atom           *type;
...
char           *name;
{
	...
	if (SoundReadFile(g->data, SoundNumBytes(s), s) != SoundNumBytes(s) ||
...
#define FILE_BYTES(_n)	((_n) * SoundBytesPerSample(s) * SoundNumTracks(s))
	...
	while (!done)
	{
	    ...
	    g->data = p;
	    n = SoundReadFile(((unsigned char *) g->data) + offset,
			      FILE_BYTES(CHUNK), s);
...
char          **argv;
{
	...
	XtVaSetValues(r->gainSlider, XtNvalue,
		      ...
		      NULL);
	r->currentMode = AuDeviceLineMode(AuServerDevice(g->aud,
							 r->inputDeviceNum));
	XtVaSetValues(r->mode, XtNlabel, r->currentMode == AuDeviceLineModeLow ?
		      "Line" : "Mic", NULL);
    }
    else
	XtSetSensitive(g->record, False);
