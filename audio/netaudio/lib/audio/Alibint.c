 ...
 * 
 * $NCDId: @(#)Alibint.c,v 1.16 1993/12/04 00:07:43 greg Exp $
 */
    ...
    int mode;
{
	...
	if (!pend && !aud->qlen && ++aud->conn_checker >= AUCONN_CHECK_FREQ)
	{
	    ...
	    CLEARBITS(r_mask);
	    BITSET(r_mask, aud->fd);
	    if ((pend = select(aud->fd + 1, (int *)r_mask, NULL, NULL,
			      &zero_time)) != 0)
	    {
		if (pend > 0)
		{
	...
	AuServer *aud;
{
	if (errno == EPIPE) {
	    (void) fprintf (stderr,
	"NCD-AUDIO connection to %s broken (explicit kill or server shutdown).\r\n",
			    AuServerString (aud));
	} else {
	    (void) fprintf (stderr, 
