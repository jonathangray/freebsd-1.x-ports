 ...
 *
 * $NCDId: @(#)ListBucket.c,v 1.7 1994/01/14 01:31:21 greg Exp $
 */
...
AuStatus       *ret_status;
{
    ...
    *num_buckets = rep.num_buckets;
	...
	if (!_AuLookupBucketInCache(aud, AuBucketIdentifier(&list[i])))
	{
	    if (!(l = (AuBucketAttributes *)
		  Aumalloc(sizeof(AuBucketAttributes))))
		break;

	    bcopy(&list[i], l, sizeof(AuBucketAttributes));
	    _AuAddToBucketCache(aud, l);
	}
