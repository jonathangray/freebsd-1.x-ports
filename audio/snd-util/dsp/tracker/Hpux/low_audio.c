...
ID("$Id: low_audio.c,v 1.1 1994/02/19 16:03:13 ache Exp $")
...
int s;
    ...
    index = 0;
    sbuffer = (short *)malloc(2 * channels * sample_rate);
    buffer = (char *)sbuffer;
    if (!buffer)
...
int update_frequency()
    ...
    if (ioctl(audio, AUDIO_GET_SAMPLE_RATE, &sample_rate) != -1)
        ...
        if (oldfreq != sample_rate)
            {
            sbuffer = (short *)realloc(sbuffer, 2 * channels * sample_rate);
            buffer = (char *)sbuffer;
            return sample_rate;
            }
        }
