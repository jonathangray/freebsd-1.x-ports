...
#ifndef lint
static char rcsid[] = "$Header: /a/cvs/386BSD/ports/x11/tk/tkPack.c,v 1.1 1993/08/09 01:20:46 jkh Exp $ SPRITE (Berkeley)";
#endif
...
static int		XExpansion _ANSI_ARGS_((Packer *childPtr,
			    int cavityWidth));
static int		YExpansion _ANSI_ARGS_((Packer *childPtr,
			    int cavityHeight));

/*
 *--------------------------------------------------------------
				 ...
				 * re-layed out. */
{
	...
	*parentPtr->abortPtr = 1;
	...
	if ((childPtr->side == TOP) || (childPtr->side == BOTTOM)) {
		...
		frameHeight += YExpansion(childPtr, cavityHeight);
	    }
	    ...
	    frameWidth = Tk_ReqWidth(childPtr->tkwin) + childPtr->padX
		    + childPtr->doubleBw;
	    if (childPtr->flags & EXPAND) {
		frameWidth += XExpansion(childPtr, cavityWidth);
	    }
 ...
 * XExpansion --
 *
 *	Given a list of packed children, the first of which is packed
...
static int
XExpansion(childPtr, cavityWidth)
    register Packer *childPtr;		/* First in list of remaining
					 * children. */
					 ...
					 * remaining children. */
{
     ...
     * Scan through the list, keeping a running sum of the heights of
     * all top and bottom windows and a running count of all expandable
     * top and bottom windows.  At each left or right window, and at the
     * end of the list, compute the expansion factor that seems reasonable
     * at that point.  Return the smallest factor seen at any of these
     * points.
     */

    minExpand = cavityWidth;
    numExpand = 0;
    for ( ; childPtr != NULL; childPtr = childPtr->nextPtr) {
	childWidth = Tk_ReqWidth(childPtr->tkwin) + childPtr->doubleBw
		+ childPtr->padX;
	if ((childPtr->side == TOP) || (childPtr->side == BOTTOM)) {
...
static int
YExpansion(childPtr, cavityHeight)
    register Packer *childPtr;		/* First in list of remaining
					 * children. */
					 ...
					 * remaining children. */
{
    ...
    numExpand = 0;
    for ( ; childPtr != NULL; childPtr = childPtr->nextPtr) {
	childHeight = Tk_ReqHeight(childPtr->tkwin) + childPtr->doubleBw
		+ childPtr->padY;
	if ((childPtr->side == LEFT) || (childPtr->side == RIGHT)) {
