...
static char *ident = "@(#)database.c	1.32 1/11/94";
 ...
 *
 *	fp	FILE* of database to save to.
 ...
 * Apply button on the Goodies popup.
 */
save_entry(fp, pref)
	FILE	*fp;
	int	pref;
...
save()
{
	...
	if ((fp = open_rcfile(cd->whichdb, "r+")) == NULL)
	{
		if (errno == ENOENT)
			fp = open_rcfile(cd->whichdb, "w");
		if (fp == NULL)
			return (0);
	}
