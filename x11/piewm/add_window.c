...
#ifdef XPM
	Scr->hilite_pm_isXpm = False;
#endif
    }
    if (Scr->hilitePm) {
#ifdef XPM
	if (Scr->hilite_pm_isXpm == False) {
#endif
	    ...
	    pm = XCreatePixmap (dpy, tmp_win->title_w,
            ...
            gc = XCreateGC (dpy, pm,
                            ...
                            &gcv);
            if (gc) {
                XCopyArea (dpy, Scr->hilitePm, pm, gc, 0, 0,
                            Scr->hilite_pm_width, Scr->hilite_pm_height,
                            0, 0);
                XFreeGC (dpy, gc);
            } else {
     ...
     TwmWindow *tmp_win;
{
    ...
    XGetWindowProperty (dpy, tmp_win->w, _XA_TWM_USER_MENU, 0L, 200L, False,
			XA_STRING, &actual_type, &actual_format, &nitems,
			&bytesafter, (unsigned char **)&tmp_win->user_menu);

    if (tmp_win->user_menu != NULL) {
        printf("Got user menu \"%s\" from window \"%s\".\n",
	       tmp_win->user_menu,
	       tmp_win->full_name);
    }
