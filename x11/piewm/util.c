...
char *name;
{
    ...
    if ((Scr->UnknownPm = FindPixmap(name, &JunkWidth, &JunkHeight, &isXpm,
				     NULL, &shape_mask)) != None)
    {
...
#ifdef SHAPE
	if (shape_mask)
	    Scr->UnknownPmmsk = Scr->UnknownPmclp = shape_mask;
	else {
	    Scr->UnknownPmmsk = SetIconMask(name);
	    Scr->UnknownPmclp = SetIconClip(name);
	    Scr->UnknownPmmsk = (Pixmap)LookInNameList(Scr->Iconsmask, name);
	}
#endif
    }
#ifdef SHAPE
    ...
    {
	Scr->UnknownPmmsk = None; 
	Scr->UnknownPmclp = None;
    }
#endif
...
Pixmap FindPixmap (name, widthp, heightp, isXpm, bg_color, shape_mask)
    char *name;
    ...
    Pixmap *shape_mask;
{
    ...
    *isXpm = False;
    ...
    pm = XcprLocatePixmapFile (ScreenOfDisplay(dpy, Scr->screen), bigname,
			       ...
			       Scr->d_depth, NULL, 0, widthp, heightp,
			       &HotX, &HotY, isXpm, &attribs, shape_mask);
    if (bg_color && *isXpm)
...
Pixmap GetPixmap (name)
    char *name;
{
    return FindPixmap (name, &JunkWidth, &JunkHeight, JunkIsXpm, NULL, NULL);
}
