...
short middle;
status_$t status;
gpr_$keyset_t keyset = { 4294967295, 4294967295, 4294967295, 4294967295, 
...
char **argv;
{
	...
	exit( 1 );
    rle_dflt_hdr.rle_file = rle_open_f("getap", fname, "r");

    switch( (int) rle_get_setup( &rle_dflt_hdr ) )
    {
    case 0:
...
/*    RLE_CLR_BIT( rle_dflt_hdr, RLE_ALPHA );	/* No alpha channel */
    ...
    rle_row_alloc( &rle_dflt_hdr, &rows );

    if (borrow_mode)
    {
	borrow_setup();
	borrow_color_map(); 
	if (rle_dflt_hdr.ymax > 511)
	    rle_dflt_hdr.ymax = 511;
    }
    else
    {
	setup_scr(rle_dflt_hdr.xmax, rle_dflt_hdr.ymax);
	window_color_map();
	if (rle_dflt_hdr.ymax > RASTERSIZE)
	    rle_dflt_hdr.ymax = RASTERSIZE;
    }
	...
	gpr_$inq_text_extent( *text, (short) strlen(text),
			      size, status );
	middle = (rle_dflt_hdr.xmax/2) - (size.x_size/2);
	if (middle < 0) middle=0;
	gpr_$move( middle, (short)(rle_dflt_hdr.ymax-15), status);
...
borrow_color_map()
{
    ...
    if ( rle_dflt_hdr.ncmap )	/* Must read color map */
    {
	...
	for (i=0; i < 256; i++)
	{
	    ...
	    value = (((rle_dflt_hdr.cmap[i]>> 8) & 0xFF) << 16);
	    value |= (((rle_dflt_hdr.cmap[i+map_offset] >> 8) & 0xFF) << 8);
	    value |= ((rle_dflt_hdr.cmap[i+2*map_offset] >> 8) & 0xFF);
	    colmap[i] = value;
	}
    }
    else
...
window_color_map()
{
	...
	for (i=0; i < 256; i++)
	{
	    ...
	    map[0][i] = ((rle_dflt_hdr.cmap[i]>> 8) & 0xFF);
	    map[1][i] = ((rle_dflt_hdr.cmap[i+map_offset] >> 8) & 0xFF);
	    map[2][i] = ((rle_dflt_hdr.cmap[i+2*map_offset] >> 8) & 0xFF);
	}
    }
    else
    {	  
...
int y;
{
    ...
    for (i = rle_dflt_hdr.xmin; i <= rle_dflt_hdr.xmax; i++)
    {
...
int y;
{
    ...
    dest_box.window_base.x_coord = rle_dflt_hdr.xmin;
    dest_box.window_base.y_coord = rle_dflt_hdr.ymax - y;
    dest_box.window_size.x_size = rle_dflt_hdr.xmin + rle_dflt_hdr.xmax + 1;
    dest_box.window_size.y_size = 1;
