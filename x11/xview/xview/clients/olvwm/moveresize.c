    ...
    char *proto;
{
	...
	if (changed & XValue)
		...
		x = DisplayWidth(dpy, scrinfo->screen) + x;
	    else ;
	else x = 0;
	if (changed & YValue)
	    if (changed & YNegative)
		y = DisplayHeight(dpy, scrinfo->screen) + y;
	    else ;
	else y = 0;
    }
...
{
    doConfigOneWindow(cli->framewin, mstuff);
    doConfigOneWindow(cli->iconwin, mstuff);
    cli->flags |= CLMoved;
    return (void *) 0;
}
...
int	dw, dh;
int 	offX, offY;
int	newScreenX, curScreenX,	newScreenY, curScreenY;
Client	*leader;
