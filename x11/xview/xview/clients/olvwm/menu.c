    ...
    Menu       *menu;
{
    ...
    int		heightofbuttons;	/* Height of all buttons */
    Graphics_info *gisButton = WinGI(winInfo, BUTTON_GINFO);
    Graphics_info *gisNormal = WinGI(winInfo, NORMAL_GINFO);
    int buttonheight = Button_Height(gisButton) + BUTT_VSPACE;
	...
	switch(pb->label[pb->which].kind) {
		...
		break;
	    case StringLabel:
		labelWidth = XTextWidth(GRV.ButtonFontInfo,
					pb->label[pb->which].string,
					strlen(pb->label[pb->which].string));
    ...
    XEvent     *event;
{
	...
	break;
      default:
	fputs(gettext("olvwm: wrong event type passed to menuSearch\n"), stderr);
	return (MenuInfo *) 0;
    }
    ...
    ScreenInfo *scrInfo;
{
	...
	(void) MenuInfoCreate(menuCache, scrInfo->rootwin,
