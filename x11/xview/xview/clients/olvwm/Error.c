...
XErrorEvent	*event;
{
	...
	if (hyperSensitive != NULL)
	{
		...
		(void) fprintf(stderr, gettext("olvwm: Fatal X Error: %s\n"), buffer);
	...
	{
               (void) fprintf(stderr, gettext("olvwm: Warning, X Error: %s\n"),
                              buffer);
...
char *txt;
{
        (void) fprintf(stderr, gettext("olvwm: Fatal Error: %s\n"), txt);
#ifdef DEBUG
	abort();
	/*NOTREACHED*/
#else
...
char *txt;
{
        (void) fprintf(stderr, gettext("olvwm: Warning: %s\n"), txt);
}
