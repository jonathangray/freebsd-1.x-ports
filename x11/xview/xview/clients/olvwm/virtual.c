...
extern Pixmap	VirtualGetPixmap();
...
int	dw = DisplayWidth(dpy, vdm->client->screen);
	...
	case ACTION_GOTO_10:
	    deltas.delta_x = vdm->offsetX +
				((a - ACTION_GOTO_1) % vdm->columns) * dw;
	    deltas.delta_y = vdm->offsetY +
				((a - ACTION_GOTO_1) / vdm->columns) * dh;
	    break;
	default:
	    return False;
    }
    ...
    WinGenericFrame	*frame;
{
    ...
    cli->wmHints->icon_pixmap = XCreateBitmapFromData(cli->dpy,
				RootWindow(cli->dpy, cli->screen), vdm_bits,
				vdm_width, vdm_height);
    cli->wmHints->icon_mask = XCreateBitmapFromData(cli->dpy,
				RootWindow(cli->dpy, cli->screen), vdmmask_bits,
...
static XTextProperty	iName = {(unsigned char *) "Desktop",
	...
	attrMask |= CWBackPixmap;
	attr.background_pixmap = VirtualGetPixmap(dpy, scrInfo, rsc->background);
    }
    ...
    XSetWMProperties(dpy, w->core.self, &wName, &iName, NULL, 0,
    ...
    ScreenInfo	*scrInfo;
{
     ...
     * Make the VDMs menu
     */
    sprintf(title, "OLVWM 3.%0d", PATCHLEVELv3);
...
int	length;
    ...
    XClearWindow(win->core.client->dpy, win->core.virtual);
    XDrawImageString(win->core.client->dpy, win->core.virtual,
		     (focus) ? win->core.client->scrInfo->gc[VDM_INPUT_GC] :
			       win->core.client->scrInfo->gc[VDM_GC],
		     1, win->core.client->scrInfo->vdm->max_ascent + 1,
...
Display	*dpy = cli->dpy;
    ...
    if (cli->framewin) {
	VIUninstallInfo(cli->framewin->core.virtual);
	VIUninstallInfo(cli->iconwin->core.virtual);
...
/*
 * Get a background pixmap for the VDM with the VDM colors
 */
Pixmap
VirtualGetPixmap(dpy, scrInfo, name)
    Display	*dpy;
    ScreenInfo	*scrInfo;
    char	*name;
...
GC	gc;
    ...
    if (XReadBitmapFile(dpy, scrInfo->rootid, name,
			&w, &h, &read, &dummy, &dummy) == BitmapSuccess) {
	write = XCreatePixmap(dpy, scrInfo->rootid, w, h, scrInfo->depth);
	gcv.function = GXcopy;
	gc = XCreateGC(dpy, scrInfo->rootid, GCFunction, &gcv);
	XSetForeground(dpy, gc, scrInfo->colorInfo.virtualPixmapColor);
	XSetBackground(dpy, gc, scrInfo->colorInfo.virtualBgColor);
	XCopyPlane(dpy, read, write, gc, 0, 0, w, h, 0, 0, 1);
	XFreePixmap(dpy, read);
	XFreeGC(dpy, gc);
	return write;
    }
    else {
	ErrorWarning(gettext("An invalid bitmap file was named as the virtual background"));
	return None;
    }
}
...
VirtualDesktop	*vdm;
	...
	else mask |= ~(CWSibling|CWStackMode);
    }
