/*
 * $Id: riseset.c,v 1.1 1993/08/17 09:42:31 alm Exp $
 */
/*
 * riseset.c
 *
...
char	*tdate, *gmctime(), *asctime(), *timezone();
struct	timeval tvp;
struct	timezone tzp;
...
#define SRATE	1.033863192	/* ratio of Moon's motion to Sun's motion */
	...
	ltm = localtime(&t1);
	sprintf(rbuf, "%02d:%02d %s", ltm->tm_hour, ltm->tm_min, timezone(tzp.tz_minuteswest, ltm->tm_isdst));
	return(rbuf);
}
