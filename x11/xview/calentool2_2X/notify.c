/*
 * $Id: notify.c,v 1.1 1993/08/17 09:42:30 alm Exp $
 */
/*
...
Event *event;
{
	...
	if (event_id(event) == MS_LEFT) {
		rtn = parse_date((char *) xv_get(setdate_pi, PANEL_VALUE),
				FALSE);
		if (!rtn)  {
			mainsw_state = DISPLAYING_DAY;
			xv_set(xv_get(canvas, CANVAS_NTH_PAINT_WINDOW, 0),
...
Notify_event_type type;
{
	...
	Notify_value value;
	void sframe_done(), mframe_done(), repaint_canvas();
