/*
 * $Id: calentool.c,v 1.1 1993/08/17 09:42:28 alm Exp $
 */
/*
 * calentool.c
 *
...
char *argv[];
{
	...
	while ((flag = getopt(argc, argv, "1:2:567bBd:eEf:iI:l:m:M:op:P:rtu:zH:h:wx:T:s:S:N:")) != EOF)
		switch (flag) {   
	...
	*/
	...
	/* create an generic Pixmap for the current icon image */
	...
	myWMHints->flags = InputHint | IconWindowHint | StateHint;
...
set_fgbg()
{
	...
	if (defaults_exists("window.color.foreground", NULL)) {
		def_str = defaults_get_string("window.color.foreground", NULL, NULL);
		def_to_xvsc(def_str, &ct_fore, &foregr);
	} else if (defaults_exists(buff, "Calentool.color.foreground")) {
	...
	if (defaults_exists("window.color.background", NULL)) {
		def_str = defaults_get_string("window.color.background", NULL, NULL);
		def_to_xvsc(def_str, &ct_back, &backgr);
	} else if (defaults_exists(buff, "Calentool.color.background")) {
...
get_user_defaults()
{
	...
	day_first = defaults_get_boolean(buff, "Calentool.dayFirst", DAY_FIRST);
