...
Pixel std_pix,focus_pix,sticky_pix;
Pixel std_fore_pix,focus_fore_pix,sticky_fore_pix;
...
void initialize_pager(void)
{
  ...
  extern char *HiBack, *StdBack, *StickyBack;
  extern char *HiFore, *StdFore,*StickyFore;
  int n,m,w,h,i,x,y;
  XGCValues gcv;
  unsigned long gcm;
  XClassHint class1;
  ...
  std_pix = GetColor(StdBack);
  if(StickyBack != NULL)
    sticky_pix = GetColor(StickyBack);
  else
    sticky_pix = std_pix;

  focus_fore_pix = GetColor(HiFore);
  std_fore_pix = GetColor(StdFore);
  if(StickyFore != NULL)
    sticky_fore_pix = GetColor(StickyFore);
  else
    sticky_fore_pix = std_fore_pix;
  ...
  if(windowFont != NULL)
    {
      ...
      gcv.foreground = std_fore_pix;
      gcv.background = std_pix;
      ...
      gcv.foreground = sticky_fore_pix;
      gcv.background = sticky_pix;
      StickyGC = XCreateGC(dpy, Scr.Root, gcm, &gcv);  
    }
}
...
void AddNewWindow(PagerWindow *t)
{
  ...
  valuemask = (CWBackPixel | CWBorderPixel | CWEventMask);
  if(t->flags & STICKY)
    attributes.background_pixel = sticky_pix;
  else
    attributes.background_pixel = std_pix;
  attributes.border_pixel = fore_pix;
  ...
  if(Scr.d_depth < 2)
      ...
      else
	{
	  if(t->flags & STICKY)
	    {
	      if(t->PagerView != None)
		XSetWindowBackground(dpy,t->PagerView,sticky_pix);     
	      XSetWindowBackground(dpy,t->IconView,sticky_pix);     
	    }
	  else
	    {
	      if(t->PagerView != None)
		XSetWindowBackground(dpy,t->PagerView,std_pix);     
	      XSetWindowBackground(dpy,t->IconView,std_pix);     
	    }
	}
    }
  if(t->PagerView != None)
