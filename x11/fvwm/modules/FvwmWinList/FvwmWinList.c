...
int Clength,Transient=0,Pressed=0,ButPressed;
ButtonArray buttons;
List windows,skipped;
char *ClickAction[3]={"Iconify -1,Raise","Iconify","Lower"},*EnterAction,
      *BackColor = "white",*ForeColor = "black", **SkipList,*geometry="";
char *font_string = "fixed";
int UseSkipList=0,SkipListLen=0,Anchor=1,UseIconNames=0;
...
/******************************************************************************
  ProcessMessage - Process the message coming from Fvwm
    Skeleton based on processmessage() from FvwmIdent:
      Copyright 1994, Robert Nation and Nobutaka Suzuki.
******************************************************************************/
int ProcessMessage(unsigned long type,unsigned long *body)
{
int redraw=0,i;
long flags;
char *name,*string;
List *temp=NULL;
  ...
  switch(type) {
    ...
    case M_CONFIGURE_WINDOW:
      ...
      if (FindItem(&skipped,body[0])!=-1) break;
      AddItem(&windows,body[0],body[8]);
      break;
    case M_DESTROY_WINDOW:
      if (DeleteItem(&skipped,body[0])!=-1) break;
    ...
    case M_RES_NAME:
    case M_RES_CLASS:
      if (UpdateItemName(&skipped,body[0],(char *)&body[3],type)!=-1) break;
      if ((i=UpdateItemName(&windows,body[0],(char *)&body[3],type))==-1) break;
      if (InSkipList((char *)&body[3])) CopyItem(&skipped,&windows,i);
      else {
        if (CompleteItem(&windows,i)) {
          string=ItemName(&windows,i,(UseIconNames)?M_ICON_NAME:M_WINDOW_NAME);
    ...
    case M_DEICONIFY:
    case M_ICONIFY:
      if ((i=FindItem(&windows,body[0]))!=-1) temp=&windows;
      else if(FindItem(&skipped,body[0])!=-1) temp=&skipped;
      else break;
      flags=ItemFlags(temp,body[0]);
      if (type==M_DEICONIFY && !(flags&ICONIFIED)) break;
      if (type==M_ICONIFY && flags&ICONIFIED) break;
      flags^=ICONIFIED;
      UpdateItemFlags(temp,body[0],flags);
      if (temp==&skipped) break;
      string=ItemName(&windows,i,(UseIconNames)?M_ICON_NAME:M_WINDOW_NAME);
...
int i=0;
  /*
  XDrawLine(dpy,win,hilite,0,0,15,0);
  XDrawLine(dpy,win,hilite,0,0,0,win_height);
  XDrawLine(dpy,win,hilite,1,1,14,1);
  XDrawLine(dpy,win,hilite,1,1,1,win_height-1);

  XDrawLine(dpy,win,shadow,15,1,15,win_height);
  XDrawLine(dpy,win,shadow,0,win_height,15,win_height);
  XDrawLine(dpy,win,shadow,14,2,14,win_height-1);
  XDrawLine(dpy,win,shadow,0,win_height-1,14,win_height-1);*/
...
FILE *ptr;
  ...
  if(ptr != (FILE *)NULL) {
    ...
    while(tline != (char *)0) {
      ...
      if(strlen(tline)>1) {
        if(strncasecmp(tline, "WindowListSkip",14)==0)
          AddToSkipList(&tline[14]);
        if(strncasecmp(tline, CatString2(Module, "Font"),Clength+4)==0)
          CopyString(&font_string,&tline[Clength+4]);
        else if(strncasecmp(tline,CatString2(Module,"Fore"), Clength+4)==0)
          CopyString(&ForeColor,&tline[Clength+4]);
        else if(strncasecmp(tline,CatString2(Module, "Geometry"), Clength+8)==0)
          CopyString(&geometry,&tline[Clength+8]);
        else if(strncasecmp(tline,CatString2(Module, "Back"), Clength+4)==0)
          CopyString(&BackColor,&tline[Clength+4]);
        else if(strncasecmp(tline,CatString2(Module, "NoAnchor"),
            Clength+8)==0) Anchor=0;
        else if(strncasecmp(tline,CatString2(Module, "Action"), Clength+6)==0)
          LinkAction(&tline[Clength+6]);
        else if(strncasecmp(tline,CatString2(Module, "UseIconNames"),
                 Clength+12)==0) UseIconNames=1;
        else if(strncasecmp(tline,CatString2(Module, "UseSkipList"),
    ...
    if (!UseSkipList) {
      if (SkipListLen>0) free(SkipList);
      SkipList=NULL;
      SkipListLen=0;
    }
  }
    ...
    switch(Event.type) {
      ...
      case KeyPress:
        ...
        if (num==1) {
          ...
          else if (buffer[0]=='s' || buffer[0]=='s') PrintList(&skipped);
...
char *temp;
  ...
  for(i=0;i<total;i++) {
    temp=ItemName(&windows,i,(UseIconNames)?M_ICON_NAME:M_WINDOW_NAME);
...
/******************************************************************************
  AddToSkipList - Add string to the list of skipped windows
******************************************************************************/
int AddToSkipList(char *string)
{
char *temp;
  for(temp=string;isspace(*temp);temp++);
  if (SkipList==NULL) SkipList=(char **)safemalloc(sizeof(char *));
  else SkipList=(char **)realloc(SkipList,(SkipListLen+1)*sizeof(char *));
  SkipList[SkipListLen]=(char *)safemalloc(strlen(temp)+1);
  strcpy(SkipList[SkipListLen],temp);
  SkipList[SkipListLen][strlen(temp)-1]='\0';
  SkipListLen++;
}

/******************************************************************************
  InSkipList - See if string is the name of a skipped window
******************************************************************************/
int InSkipList(char *string)
{
int i;
  for(i=0;i<SkipListLen;i++) {
    if (!strcasecmp(SkipList[i],string)) break;
  }
  return(i<SkipListLen);
}

/******************************************************************************
  PrintSkipList - Print the list of skipped windows on the console (Debugging)
******************************************************************************/
void PrintSkipList(void)
{
int i;
  ConsoleMessage("WindowSkipList:\n");
  for(i=0;i<SkipListLen;i++) ConsoleMessage("    %s\n",SkipList[i]);
}
