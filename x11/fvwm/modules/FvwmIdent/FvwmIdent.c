...
void main(int argc, char **argv)
{
  ...
  if(file != (FILE *)NULL)
      ...
      while(tline != (char *)0)
	{
	  ...
	  if(strlen(tline)>1)
	    {
	      if(strncasecmp(tline, CatString2(MyName, "Font"),Clength+4)==0)
		{
		  CopyString(&font_string,&tline[Clength+4]);
		}
	      else if(strncasecmp(tline,CatString2(MyName,"Fore"),
				  Clength+4)==0)
		{
		  CopyString(&ForeColor,&tline[Clength+4]);
		}
	      else if(strncasecmp(tline,CatString2(MyName, "Back"),
				  Clength+4)==0)
		{
...
/*************************************************************************
 *
 * Concatenates two strings into a scratch buffer 
 * 
 ************************************************************************/
char *CatString2(char *a, char *b)
{
  char CatS[100];
  if (strlen(a)+strlen(b) > 99)
    return NULL;
...
void MakeList(void)
{
  ...
  if(x1 < 0)
    x1 = 0;
  x2 = ScreenWidth - x1 - target.frame_w - 2;
  if(x2 < 0)
    x2 = 0;
  y1 = target.frame_y;
  if(y1 < 0)
    y1 = 0;
  y2 = ScreenHeight - y1 -  target.frame_h - 2;
    if(y2 < 0)
    y2 = 0;
