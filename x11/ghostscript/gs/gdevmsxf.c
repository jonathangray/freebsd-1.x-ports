...
{	win_xfont f;
	...
	if ( f.lf.lfHeight <= 0 || f.lf.lfHeight >= 36 )
		return NULL;
	f.lf.lfWidth = 0;
	f.lf.lfEscapement = 0;
	...
	wf = (win_xfont *)(*mprocs->alloc)(1, sizeof(win_xfont), "win_lookup_font");
	if ( wf == 0 )
...
/* Release an xfont. */
private int
win_release(gx_xfont *xf, const gs_memory_procs *mprocs)
{	if ( wxf->hFont )
	{	DeleteObject(wxf->hFont);
		wxf->hFont = 0;
	}
	if ( mprocs != NULL )
		(*mprocs->free)((char *)xf, 1, sizeof(win_xfont),
				"win_release");
	return 0;
}
