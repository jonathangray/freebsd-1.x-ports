 ...
 * give the actual restrictions, but the Ghostscript kernel requires that
...
{	uint raster = gdev_prn_raster(pdev);
	...
	{	gdev_prn_copy_scan_lines(pdev, lnum, data, raster);
		code = (*row_proc)(pdev, data, pdev->color_info.depth, pstream);
		if ( code < 0 ) break;
	}
