...
!!! test -f /news/lib/innd/innd.pid && echo 0 || echo 1 ! eq ! 1 ! exit ! innd dead
...
!hiload!+ load! uptime | tr -d ,. | awk '{ print $(NF - 2) }' ! gt ! 5000 ! throttle ! loadav
##  =()<!load!+! uptime | tr -d ,. | awk '{ print $(NF - 2) }' ! gt ! @<INNWATCH_PAUSELOAD>@ ! pause ! loadav>()=
!load!+! uptime | tr -d ,. | awk '{ print $(NF - 2) }' ! gt ! 4000 ! pause ! loadav
...
!!! df . | awk 'NR == 2 { print $4 }' ! lt ! 3000 ! throttle ! No space (spool)
##  =()<!!! df @<_PATH_BATCHDIR>@ | awk 'NR == 2 { print $4 }' ! lt ! @<INNWATCH_BATCHSPACE>@ ! throttle ! No space (newsq)>()=
!!! df /news/spool/out.going | awk 'NR == 2 { print $4 }' ! lt ! 1000 ! throttle ! No space (newsq)
