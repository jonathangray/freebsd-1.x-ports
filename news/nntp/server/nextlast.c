#ifndef lint
static char	*sccsid = "@(#)$Header: /a/cvs/386BSD/ports/news/nntp/server/nextlast.c,v 1.1 1993/07/19 20:04:31 nate Exp $";
#endif
	...
	char	*argv[];
{
	...
	if (next ? (art_ptr + 1 >= num_arts) : (art_ptr - 1 < 0)) {
		...
		next ? ERR_NONEXT : ERR_NOPREV,  next ? "next" : "previous");
		(void) fflush(stdout);
		return;
	}
