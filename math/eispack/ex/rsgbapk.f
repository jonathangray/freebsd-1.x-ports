...
c     some of the eigenvalues for the eigenproblem  b*a*x = (lambda)*x.
c
c     this driver is catalogued as  eispdrv4(rsgbapk).
c
...
c
  ...
  715    format(//30h *****error from rsgba***** = ,i4)
...
c
  ...
  806    format(//30h *****error from rsgba***** = ,i4)
         if( error .eq. 7*n + 1 ) go to 230
         maxeig = 0.0d0
         maxdif = 0.0d0
         m = n
...
c
 ...
 1015    format(//31h *****error from reduc2***** = ,i4)
  ...
  102    format(//31h *****error from bisect***** = ,i4)
         go to  150
c
c     rsgba1w1z  using  tridib  and  tinvit
c
...
c
  ...
  190    call  rmatin(nm,n,a,b,ahold,bhold,1)
         call  rsgabr(nm,n,m,b,a,w,z,norm,resdul,rv2)
  ...
  212       write(iwrite,213) (w(k),k=kk,kk1),(norm(k),k=kk,kk1),
     x           ((z(i,k),k=kk,kk1),i=1,n)
  213    format(///1x,3(5x,24heigenvalue of b*a-lambda,3x)/
     x   1x,3(4x,1pd24.16,4x)//
     x   1x,3(5x,23h1-norm of corresponding,4x)/
     x   1x,3(9x,15hresidual vector,8x)/
     x   1x,3(7x,19h!!b*a*z-z*diag(w)!!,6x)/
     x   1x,3(5x,22h----------------------,5x)/
     x   1x,3(8x,17h!!b!!*!!a!!*!!z!!,7x)//
     x   1x,3(8x,d16.8,8x)//
