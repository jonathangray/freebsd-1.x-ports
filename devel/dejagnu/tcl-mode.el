;; Appended is the third beta release of tcl.el.  This time I made sure I
;; got the right file before sending :-).

;; New in this release:

;; * Menu support for Lucid and GNU Emacs 19.  Bind 'tcl-popup-menu to
;; some mouse button if you want a popup menu.  Put
;;	(tcl-install-menubar)
;;  in your tcl-mode-hook if you want a "Tcl" menu on the menubar.
;;  The menu code was lifted more-or-less verbatim from the "other Tcl
;;  mode" by schmid@fb3-s7.math.TU-Berlin.DE (Gregor Schmid).

;;* There is now a function tcl-restart-with-file that will restart the
;;  inferior process and feed it a file as initial input.

;;* tcl-guess-application can be used to set tcl-application based on
;;  the "#!" line at the beginning of the file.  (Put this in your
;;  tcl-mode-hook if you like).  This was inspired by code in the "other
;;  Tcl mode".

;;Some notes:

;;* I still haven't integrated the Emacs 18 patches.  Anyone using Emacs
;;  18 can get them from me.

;;* There is a report that font-lock-mode behaves strangely in Lucid
;;  Emacs 19.  Namely on a line like "proc foo {} {", the space before
;;  "foo" is highlighted like "foo".  I don't see this in FSF 19, and I
;;  don't have Lucid 19 for testing.  A patch would be appreciated.

;;* I've tried various values for tcl-omit-ws-regexp in attempts to get
;;  end-of-defun to work properly in FSF Emacs 19.  Recently I
;;  discovered that end-of-defun actually is broken.  So that explains
;;  why I couldn't make it work.  Hopefully it will work in some future
;;  version.

;;* Those of you using GNU Emacs 19 might experience weird problems when
;;  using menus.  The Lucid Emacs menu compatibility package is
;;  moderately broken.  I'd suggest getting my patches to lmenu.el from
;;  gnu.emacs.bug, or getting the patches from me directly.

;;Barring any major bugs (or feature requests I'm motivated to write),
;;I'm going to release this version to the world at large early next
;;week.
...
;; FEATURES:
;; * Moderately flexible indentation.
;; * Supports imenu, add-log, and font-lock out of the box.
;; * Electric "#" guesses if you are typing a real comment or not.
;; * Auto-filling of comments supported.
;; * Inferior Tcl mode.
;; * Interface to TclX help files.
...
(defvar tcl-lucid-menu
  '("Tcl"
    ...
    "---"
    ["Show inferior buffer" inferior-tcl t]
    ["Send function to inferior" tcl-eval-defun t]
    ["Send region to inferior" tcl-eval-region t]
    ["Send file to inferior" tcl-load-file t]
    ["Restart inferior with file" tcl-restart-with-file t]
    "---"
...
Interesting functions to run in your `tcl-mode-hook':
  tcl-auto-fill-mode
    Toggle auto-filling in comments.  This function works like
    `auto-fill-mode'.
  tcl-hashify-buffer
    Automatically quote \"#\" characters which do not correspond to
    actual comments.
  tcl-guess-application
    Guess value of `tcl-application' from the \"#!\" line of the file.
...
(defun tcl-install-menubar ()
  (and tcl-using-emacs-19
       ...
       ;;   Doubly bogus!
       ;; Hopefully both these problems will be fixed in a future
       ;; version of GNU Emacs.  I have reported them as bugs.
