...
{
    int c, dribbling, vbar = 0;
    char *phys_line, p_line[MAX_PHYS_LINE];
...
NODE *runparse_node(NODE *nd, NODE **ndsptr)
{
	...
	/* minus sign with space before and no space after is unary */
	    ...
	    tcnt = 0;
	    tptr = wptr;
	    /* isnumb 0 means digits so far, 1 means just saw
