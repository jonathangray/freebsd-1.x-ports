...
structure LexGen: LEXGEN =
 ...
 (*open Array List*)
   ...
   datatype token = CHARS of bool Array.Array | QMARK | STAR | PLUS | BAR
   ...
   datatype exp = EPS | CLASS of bool Array.Array * int | CLOSURE of exp
   ...
   val say = fn x => Outstream.write (!LexOut) x
...
with
	...
	fun getch (a as (BUF(s,{b,p}))) = 
		 if (!p = (size (!b)))
		   then (b := Instream.input(s, 1024);
			 p := 0;
			 if (size (!b))=0
			    then raise eof 
			    else getch a)
		   else (let val ch = String.extract (!p) (!p+1) (!b)
			 in (if ch = "\n"
...
val pr_err = fn x => (Outstream.write std_out ("mlex: syntax error in line "^
			    (Int.string (!LineNum))^
			    ": "^x^"\n"); raise error)
...
val rec skipws = fn () => case nextch() of
	...
	and onechar = fn x => let val c = Array.create (!CharSetSize) false in
		Array.update(c,ord(x),true); CHARS(c)
		end
...
****)
	...
	| 1 => let val rec makeTok = fn () =>
		...
		| "/" => SLASH
		| ";" => SEMI
		| "." => let val c = Array.create (!CharSetSize) true in
		...
		| "[" => let val rec classch = fn () => let val x = skipws()
			...
			val first = classch();
			val flag = (first<>"^");
			val c = Array.create (!CharSetSize) (not flag);
			val rec add = fn x => if x="" then ()
				else Array.update(c,ord(x),flag)
			and range = fn (x,y) =>
...
val SymTab = ref (create(General.uncurry AsciiOrdString.le)) : (string,exp) dictionary ref

fun GetExp () : exp =

	let val rec optional = fn e => ALT(EPS,e)

	and newline = fn () => let val c = Array.create (!CharSetSize) false in
...
val StateTab = ref(create(General.uncurry AsciiOrdString.le)) : (string,int) dictionary ref 
...
fun parse() : (string * (int list * exp) list * ((string,string) dictionary)) =
	let val Accept = ref (create(General.uncurry AsciiOrdString.le)) : (string,string) dictionary ref
		   ...
		   (LexState:=2; AdvanceTok();
		    case GetTok() of ACTION(act) =>
		      if !NextTok=SEMI then
		        (Accept:=enter(!Accept) (Int.string (!LeafNum),act);
...
structure L = 
	struct
	  ...
	  fun > ((key,item:string),(key',item')) =
	    let fun f ((a:int)::a') (b::b') = if Int.gt a b then true
...
(* Fins = (state #, list of final leaves for the state) list
   ...
   let datatype elem = N of int | T of int | D of int
       val count = ref 0
       val _ = (if List.size(trans)<256 then CharFormat := true
		 else CharFormat := false;
		 if List.size(tcpairs)> 0 then 
		 ...
		 case !CharFormat of
		     ...
		     | false => say "int Vector.Vector}";
				(* NICK: SML Library uses type `Vector' *)
	 ...
	 let open RB
	     fun makeItems x =
	       let fun MakeList(nil,i) = ()
	             | MakeList([(x:int)],i) = say (Int.string x)
		     | MakeList(x::tl,16) =
		       (say "\n"; say (Int.string x); say ","; MakeList(tl,1))
	             | MakeList(x::tl,i) =
		       (say (Int.string x); say ","; MakeList(tl,i+1))
	           fun MakeString(nil,i) = ()
		     | MakeString(((x:int)::tl),i) =
		         let val x = (Int.string x)
	...
	fun msg x = Outstream.write std_out x

  in (say "in Vector.fromList\n["; makeTable(rs,newfins); say "]\nend\n";
    msg ("\nNumber of states = " ^ (Int.string (List.size trans)));
    msg ("\nNumber of distinct rows = " ^ (Int.string (!count)));
    msg ("\nApprox. memory size of trans. table = " ^
	  (Int.string (!count*(!CharSetSize)*(if !CharFormat then 1 else 8))));
    msg " bytes\n")
end
...
fun leafdata(e:(int list * exp) list) =
	let val fp = Array.create (!LeafNum + 1) nil
	and leaf = Array.create (!LeafNum + 1) EPS
	and tcpairs = ref nil
	and trailmark = ref ~1;
	val rec add = fn
		  (nil,x) => ()
...
fun makedfa(rules) =
let val StateTab = ref (create(General.uncurry AsciiOrdString.le)) : (string,int) dictionary ref
    val fintab = ref (create(General.uncurry Int.le)) : (int,(int list)) dictionary ref
    val transtab = ref (create(General.uncurry Int.le)) : (int,int list) dictionary ref
    val tctab = ref (create(General.uncurry Int.le)) : (int,(int list)) dictionary ref
...
and startstates() =
	let val startarray = Array.create (!StateNum + 1) nil;
            fun listofarray(a,n) =
...
fun lexGen(infile) =
    ...
    let val sayln = fn x => (say x; say "\n")
	 ...
	 sayln "\t\t\t(let val yytext = String.extract i0 i (!yyb)\n\
	       \\t\t\t     val yypos = i0+ !yygone"; (* NICK: String.extract. *)
	 if !CountNewLines 
	 ...
	 sayln "\t\t     else yyb := String.extract i0 l (!yyb)^newchars;";
					(* NICK: String.extract *)
	 sayln "\t\t     yygone := !yygone+i0;";
	 sayln "\t\t     yybl := size (!yyb);";
	 sayln "\t\t     scan (s,AcceptingLeaves,l-i0,0))";
	 sayln "\t    end";
	 sayln "\t  else let val NewChar = String.ord(String.nth l (!yyb))";
	   say "\t\tval NewState = ";
	   case (!CharFormat,!CharSetSize)
	    of (true,129) => sayln "if NewChar<128 then String.ord(String.nth NewChar trans) else String.ord(String.nth 128 trans)"
	     | (true,256) => sayln "String.ord(String.nth NewChar trans)"
	 ...
	 sayln "\tval start= if String.extract (!yybufpos-1) (!yypufpos) (!yyb)=\"\\n\"";
					(* NICK: String.extract. *)
	...
	StateNum := 2;
	LineNum := 1;
	StateTab := enter(create( op<=))("INITIAL",1);
	LeafNum := ~1;
	let
