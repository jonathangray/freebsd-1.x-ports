...
functor ParseGenFun(structure ParseGenParser : PARSE_GEN_PARSER
  ...
  (*open Array List*)
    ...
    structure SymbolHash = Hash(type elem = string
	    		        val gt = General.uncurry AsciiOrdString.gt
			       )
    ...
    *)
	...
	List.apply prConstr term;
	List.apply prConstr nonterm;
	sayln "\nend";
    ...
    val printEC = fn (keyword : term list,
	 ...
	 val subst =
	   ...
	   in TermTable.make_list (List.foldR (General.curry f) TermTable.empty subst)
	   end

	 val sayterm = fn (T i) => (say "(T "; say (Int.string i); say ")")

	 val printBoolCase = fn ( l : term list) =>
	    (say "fn ";
	     List.apply (fn t => (sayterm t; say " => true"; say " | ")) l;
	     sayln "_ => false")

	 val printTermList = fn (l : term list) =>
	    (List.apply (fn t => (sayterm t; say " :: ")) l; sayln "nil")
	 ...
	 val printSubst = fn (l : (term * (term list)) list) =>
	    (sayln "val preferred_subst =";
	     ...
	     List.apply (fn (t,l') =>
		    (sayterm t; say " =>";
		     List.apply (fn t => (sayterm t; say "::")) l';
		     sayln "nil"; say "|"
		    )
		 ) l;
	     sayln " _ => nil")
...
let val printAbsynRule = Absyn.printRule(say,sayln)
    ...
    val printCase = fn (i:int, r as {lhs=lhs as (NT lhsNum),prec,
	   ...
	   val mkToken = fn (sym,num : int,typed) =>
	     let val symString = symbolToString sym
	       val symNum = symString ^ (Int.string num)
	     in PTUPLE[WILD,
	...
	(* remove terminals in argument list w/o types *)
	   ...
	   val argsWithTypes =
		  List.foldR (General.curry(fn ((_,_,false),r) => r
			   ...
			   | r =>let val (rsym,rnum,_) = hd(numberedRhs)
				 ...
				 in ETUPLE[resultexp,
					   EVAR (symbolToString lsym ^
						 Int.string lnum ^ "left"),
					   EVAR (symbolToString rsym ^
						  Int.string rnum ^ "right")]
				 end,
                           rest])
	...
	val termHash = 
	  List.foldR (fn (symbol,_) => fn table =>
...
*)
	...
	val termToString =
	   let val data = Array.create numTerms ""
	       ...
	       val _ = List.apply unmap term
	   in fn T i =>
		if DEBUG andalso (i<0 orelse i>=numTerms)
		  then "bogus-num" ^ (Int.string i)
		  else data sub i
	   end

	val nontermToString = 
	   let val data = Array.create numNonterms ""
	       val unmap = fn (symbol,_) =>
		    let val name = symbolName symbol
		    in Array.update(data,
			      case SymbolHash.find(name,symbolHash)
	       ...
	       val _ = List.apply unmap nonterm
	   in fn NT i =>
...
precedences of the rule and the terminal are equal.
  ...
  A rule is given the precedence of its rightmost terminal *)
	...
	val termPrec =
	    let val precData = Array.create numTerms (None : int Option)
		...
		val termPrec = fn ((LEFT,_) ,i) => i
			      | ((RIGHT,_),i) => i+2
			      | ((NONASSOC,l),i) => i+1
		val _ = List.foldL (fn (x, l) => fn i =>
			        (List.apply (addPrec (Some (termPrec ((x,l),i)))) l; i+3))
			  0 prec
	   in fn (T i) =>
	...
	val symbolType = 
	   let val data = Array.create (numTerms+numNonterms) (None : ty Option)
	       val unmap = fn (symbol,ty) =>
		      Array.update(data,
			     case SymbolHash.find(symbolName symbol,symbolHash)
			     of Some i => i,ty)
	       val _ = (List.apply unmap term; List.apply unmap nonterm)
	   in fn NONTERM(NT i) =>
		if DEBUG andalso (i<0 orelse i>=numNonterms)
		  then None
	     ...
	     val printRule =
		   ...
		   let val prRule = fn {lhs,rhs,precedence,rulenum} =>
		     ((say o nontermToString) lhs; say " : ";
		      List.apply (fn s => (say (symbolToString s); say " ")) rhs)
