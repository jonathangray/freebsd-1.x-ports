...
functor mkCoreUtils(structure Core : CORE) : CORE_UTILS =
	...
	val mkFuncs = fn (GRAMMAR {rules,terms,nonterms,...}) =>
	   let val derives=Array.create nonterms (nil : rule list)
		...
		val _ =
		   let val f =
			...
			fun g(i,num) =
		          if i<nonterms then
			    let val (l,n) =
				        List.foldR (General.curry f) (nil, num) (derives sub i)
			    in Array.update(derives,i,List.rev l); g(i+1,n)
			    end
			  else ()
		    in g(0,0)
		    end
 ...
 *)
		...
		fun sortItems nt =
		 let fun add_item (a as RULE{rhs=symbol::rest,...},r) =
		       ...
		       | add_item (_,r) = r
		 in
		   List.foldR (General.curry add_item) Assoc.empty (produces nt)
		 end
...
(* getNonterms: get the nonterminals with a .  before them in a core.
   Returns a list of nonterminals in ascending order *)
		...
		fun getNonterms l =
		  List.foldR (General.curry (fn (ITEM {rhsAfter=NONTERM sym ::_, ...},r) =>
				NtList.insert(sym,r)
			      | (_,r) => r)) nil l
...
(* now create a table which for each shift/goto symbol gives the sorted list
   ...
   and then sorting them by the shift/goto symbols *)
		...
		val newsets = List.foldR (General.curry mergeItems) Assoc.empty  nonterms
...
(* insert the kernal items of a core *)
		...
		val newsets = List.foldR (General.curry shiftCores) newsets itemList
...
(* nontermEpsProds: returns a list of epsilon productions produced by a
   ...
   were returned by produces; it only removes non-epsilon productions *)
	   ...
	   val nontermEpsProds =
	      let val f = fn nt =>
		  List.foldR (General.curry(fn (rule as RULE {rhs=nil,...},results) =>
				rule :: results
			 | (_,results) => results)) nil (produces nt)
	       in memoize f
	       end 
...
*)
	...
	fun epsProds (CORE (itemList,state)) =
	   ...
	   in List.foldR (General.curry op @) nil prods
	   end
