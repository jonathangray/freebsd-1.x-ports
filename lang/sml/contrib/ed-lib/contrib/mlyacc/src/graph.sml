...
functor mkGraph(structure IntGrammar : INTGRAMMAR
	...
	struct
		...
		type graph = {edges: {edge:symbol,to:core} list Array.Array,
			      nodes: core list,nodeArray : core Array.Array}
		   ...
		   let val {shifts,produces,rules,epsProds} =
			 ...
			 | f (nodes,node_list,edge_list,h::t,y,num) =
			 	let val (nodes,edges,future,num) =
				   List.foldR (General.curry add_goto) (nodes,nil,y,num) (shifts h)
		...
		in {graph=
		   let val makeItem = fn (r as (RULE {rhs,...})) =>
			...
			val initialItemList = map makeItem (produces start)
		        val orderedItemList =
			   List.foldR (General.curry Core.insert) nil initialItemList
	...
	val prGraph = fn a as (nontermToString,termToString,print) => fn g =>
	   let val printCore = prCore a
	       ...
	       val nodes = nodes g
	       val printEdges = fn n => 
		 List.apply(fn {edge,to=CORE (_,state)} =>
			      (print "\tshift on ";
			       printSymbol edge;
			       print " to ";
			       print (Int.string state);
			       print "\n")) (edges (n,g))
	 in List.apply (fn c => (printCore c; print "\n"; printEdges c)) nodes
	 end
end;
