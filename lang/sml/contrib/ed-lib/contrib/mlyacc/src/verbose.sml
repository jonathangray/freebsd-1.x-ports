...
struct
   ...
   val mkPrintAction = fn print =>
	let val printInt = print o Int.string
	in fn (SHIFT (STATE i)) =>
			(print "\tshift ";
		 	 printInt i;
		 	print "\n")
   ...
   val printVerbose =
	fn {termToString,nontermToString,table,stateErrs,entries:int,
	   ...
	   let 
		...
		val _ = if List.size errs > 0 
			   then (printSummary print errs;
			         print "\n";
				 List.apply printError errs)
			   else ()  
		fun loop i =
		  if i=states then ()
		  else let val s = STATE i
		       in (List.apply printError (stateErrs s);
			   print "\n";
			   printCore s;
	  ...
	  in loop 0;
	      print (Int.string entries ^ " of " ^
		     Int.string (!actionTableSize)^
		     " action table entries left after compaction\n");
	      print (Int.string (!gotoTableSize)^ " goto table entries\n")
	  end
end;
