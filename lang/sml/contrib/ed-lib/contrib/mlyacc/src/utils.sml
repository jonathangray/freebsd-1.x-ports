...
struct
 ...
 fun revfold f list x = List.foldL (General.curry f) x list
 fun fold f list x =    List.foldR (General.curry f) x list
...
val make_set = fn l => List.foldR (General.curry insert) nil l
					(* NICK *)
...
(* equal_tree : test if two trees are equal.  Two trees are equal if
   ...
   fun set_gt (tree1,tree2) =
	 ...
	 fun getvalue(stack as ((a,position)::b)) =
      ...
      fun make_set l = List.foldR (General.curry insert) empty l
					(* NICK *)
...
struct
 ...
 fun find(key,t) =
  ...
  fun make_table l = List.foldR (General.curry insert) empty l
					(* NICK *)
