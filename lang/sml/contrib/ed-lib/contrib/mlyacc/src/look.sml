...
functor mkLook (structure IntGrammar : INTGRAMMAR) : LOOK =
	  ...
	  let fun ok_rhs nil = true
		...
		| ok_rhs ((NONTERM i)::r) = ok_rhs r
		 ...
		 if ok_rhs rhs then (lhs,map (fn (NONTERM (NT i)) => i) rhs)::r
	      ...
	      val items = List.foldR (General.curry add_rule) nil rules
	      val nullable = Array.create nonterms false
	      val f = fn ((NT i,nil),(l,_)) => (Array.update(nullable,i,true);
				 	       (l,true))
		       | (a as (lhs,(h::t)),(l,change)) =>
				case (nullable sub h) 
				  of false => (a::l,change)
				   | true => ((lhs,t)::l,true)
	      fun prove(l,true) = prove(List.foldR (General.curry f) (nil,false) l)
		| prove(_,false) = ()
	...
	symbols. *)
      ...
      fun accumulate(rules, empty, addObj) =
       List.foldR (fn (RULE {rhs,...}) => fn r =>(scanRhs addObj) (rhs,r)) empty rules

      val nontermMemo = fn f =>
	let val lookup = Array.create nonterms nil
	...
	union the whole lot together *)
      ...
      fun first nt =
	     List.foldR (fn a => fn r => TermSet.union(r,first1 a))
	     nil
