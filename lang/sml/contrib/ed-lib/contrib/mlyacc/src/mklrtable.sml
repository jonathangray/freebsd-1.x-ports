	...
	structure Errs : LR_ERRS =
		...
		datatype err = RR of LrTable.term * LrTable.state * int * int
		   ...
		   let val numRR = ref 0
		       ...
		       fun loop (h::t) = 
		       (case h
		         of RR _ => Ref.inc numRR
			  | SR _ => Ref.inc numSR
			  | NONASSOC _ => Ref.inc numNONASSOC
			  | START _ => Ref.inc numSTART
			  | NOT_REDUCED _ => Ref.inc numNOT_REDUCED
			  | NS _ => Ref.inc numNS; loop t)
...
*)
  ...
  val computeActions = fn (rules,precedence,graph,defaultReductions) =>
    ...
    let val rulePrec =
	  let val precData = Array.create (List.size rules) (None : int Option)
	  in List.apply (fn RULE {rulenum=r,precedence=p,...} => Array.update(precData,r,p))
	     rules;
	     fn i => precData sub i
	  end
  ...
  in fn (Lalr.LCORE (reduceItems,state),c as CORE (shiftItems,state')) =>
		...
		of nil => ((shifts,ERROR),gotos,nil)
		 ...
		 | l =>
		  let val (reduces,errs1) =
			 List.foldR (General.curry(mergeReduces tableState)) (nil, nil) l
		      val (actions,errs2) =
	...
	val mkTable = fn (grammar as GRAMMAR{rules,terms,nonterms,start,
		  ...
		  (* add goto from state 0 to a new state.  The new state
		   ...
		   val (actions,gotos,errs) =
			...
			let val newStateActions = 
			   ...
			   val state0Goto = 
			       GotoList.insert((start,STATE (List.size actions)),h)
		...
		val startErrs =
	 	  List.foldR (fn RULE {rhs,rulenum,...} => fn r =>
		...
		val notReduced =
		  let val ruleReduced = Array.create (List.size rules) false
		      val test = fn REDUCE i => Array.update(ruleReduced,i,true)
				  | _ => ()
		      val _ = List.apply (fn (actions,default) =>
				     (List.apply (fn (_,r) => test r) actions;
				      test default)
				  ) actions;
		      fun scan (i,r) =
			 if i >= 0 then
		  ...
		  in scan(Array.size ruleReduced-1,nil)
	    	  end handle Array.Subscript _ =>
			(if DEBUG then
				Outstream.write std_out "rules not numbered correctly!"
			 else (); nil)

		val numstates = List.size actions

	        val allErrs = startErrs @ notReduced @ nonshiftErrs @ 
			      (List.foldR (General.curry op @) nil errs)
	      ...
	      in (mkLrTable {actions=Array.fromList(map (fn (a,b) =>
			     ...
			     numRules=List.size rules,numStates=List.size actions,
