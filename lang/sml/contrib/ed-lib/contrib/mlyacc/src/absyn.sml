   ...
   struct
         ...
         val simplifyRule : rule -> rule = fn (RULE(p,e)) =>
            let val used : (string -> bool) =
               let fun f(CODE s) = code_to_ids s
                     | f(EAPP(a,b)) = f a @ f b
                     | f(ETUPLE l) = List.foldR (General.curry op @) nil (map f l)
                     | f(EVAR s) = [s]
                     | f(FN(_,e)) = f e
                     | f(LET(dl,e)) =
                          (List.foldR (General.curry op @) nil (map (fn VB(_,e) => f e) dl)) @
                           f e
                     | f(SEQ(a,b)) = f a @ f b
                     | f _ = nil
                   val identifiers = f e
     ...
     let val lp = ["("]
	   ...
	   let fun f (AS(PVAR a,PVAR b)) = [a," as ",b]
                 ...
                 | f (PAPP(a,WILD)) = [a," ","_"]
                 | f (PAPP(a,PINT i)) =  [a," ",Int.string i]
                 | f (PAPP(a,PVAR b)) = [a," ",b]
                 | f (PAPP(a,b)) = compress [lp,[a],sp,f b,rp]
	         | f (PINT i) = [Int.string i]
         ...
         in fn a => 
	      let val RULE(p,e) = simplifyRule a
              in List.apply (fn x => (oursay x; ())) (printPat p);
	         say " => ";
                 List.apply (fn x => (oursay x; ())) (printExp e)
              end
         end
end;
