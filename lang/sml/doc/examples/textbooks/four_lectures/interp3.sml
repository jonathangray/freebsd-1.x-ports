(* interp3.sml , a more efficient (?) 
   implementation of types and substitutuions *)
...
struct
  ...
  fun tc (exp: Ex.Expression): Ty.Type =
   (case exp of
    ...
    | Ex.LISTexpr(e::es) => tc (Ex.CONSexpr(e,Ex.LISTexpr es))
    | Ex.CONSexpr(e1,e2) =>
        let val t1 = tc e1
            val t2 = tc e2
...
struct
  ...
  fun mkTypeInt() = INT
  and unTypeInt(INT)=()
    | unTypeInt(TYVAR(r as(ref (SOME (TYVAR tv)))))=
       (r:= !tv; unTypeInt(TYVAR r))
  ...
  fun mkTypeBool() = BOOL
  and unTypeBool(BOOL)=()
    | unTypeBool(TYVAR(r as(ref (SOME (TYVAR tv)))))=
       (r:= !tv; unTypeBool(TYVAR r))
    | unTypeBool(TYVAR(ref (SOME t)))= unTypeBool t
    | unTypeBool(_)= raise Type

  fun mkTypeList(t)=LIST t
  and unTypeList(LIST t)= t
    | unTypeList(TYVAR(r as(ref (SOME (TYVAR tv)))))=
       (r:= !tv; unTypeList(TYVAR r))
    | unTypeList(TYVAR(ref (SOME t)))= unTypeList t
    | unTypeList(_)= raise Type

  fun mkTypeTyvar tv = TYVAR tv
  and unTypeTyvar(TYVAR (tv as (ref NONE))) = tv
    | unTypeTyvar(TYVAR(r as(ref (SOME (TYVAR tv)))))=
       (r:= !tv; unTypeTyvar(TYVAR r))
    | unTypeTyvar _ = raise Type
