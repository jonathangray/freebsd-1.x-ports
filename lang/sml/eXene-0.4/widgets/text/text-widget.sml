...
abstraction TextWidget : TEXT_WIDGET =
  struct
    structure W = Widget
  ...
  (* make a text window size descriptor from a window size and font. *)
    fun mkTextSz (sz as SIZE{wid, ht}, font) = let
	  ...
	  in
	    TSZ{
		sz = sz,
		rows = (ht - totPad) quot charHt,
		cols = (wid - totPad) quot charWid,
   ...
   *)
    local
      datatype text_line = TL of (ByteArray.bytearray * (int * int) list)
    in
	...
	fun newTextLn cols = TL(ByteArray.array(cols, ord " "), [])

      (* write a string into a bytearray starting at col. *)
	fun writeStr (ba, col, str) = let
	      fun cpy (i, j) = (ByteArray.update(ba, i, ordof(str, j)); cpy(i+1, j+1))
	      in
		(cpy(col, 0)) handle _ => ()
	      end
	      ...
	      val TL(ba, format) = Array.sub(arr, row)
              ...
              fun copyStr (0,_,_) = ()
                | copyStr (cnt,indx,inc) = (
                      ByteArray.update(ba,indx+delta,ByteArray.sub(ba,indx));
	...
	fun mkTextBuf (TSZ{rows, cols, ...}) = let
	      fun loop (0, l) = TB{sz=SIZE{wid=cols,ht=rows},arr=Array.arrayoflist l}
	...
	fun clearTextLn (TB{arr,...}, ChrCrd{row, col}) = let
	      ...
	      in
		if (col = 0)
		  then Array.update(arr, row, newTextLn(ByteArray.length ba))
		  else let
		    fun clr i = (ByteArray.update(ba, i, ord " "); clr(i+1))
		    val newFormat = insN (col, (ByteArray.length ba) - col, format)
		    in
		      (clr col) handle _ => ();
	...
	fun explodeRow {text = TB{arr=text,...}, row, col, len} = (
	      case (Array.sub(text, row))
	       of TL(ba, []) => [ByteArray.extract(ba, col, len)]
		| TL(ba, l) => let
		    val endCol = col+len
		    fun ext (col, len) = ByteArray.extract(ba, col, len)
	...
	fun resizeTextBuf (TB{arr=oldA,...}, newSz as TSZ{rows, cols, ...}) = let
	      ...
	      fun copy row = let
		    ...
		    fun cpy col = (
			  ByteArray.update(newBA, col, ByteArray.sub(oldBA, col));
			  cpy (col+1))
   ...
   *)
    ...
    (* make a text window of the given size *)
      fun mkTextWin (win, font, size) = let
	    ...
	    val (pen, highlighter, normalStipple, highlightStipple) = let
		  ...
		  val stipple = createTileFromAsciiData (EXeneWin.screenOfWin win)
			cursorStippleData
		  in
    ...
    (* create a new text window descriptor to reflect a change in the window size *)
      fun resizeTextWin (TW{win, font, ...}, newSize) = mkTextWin (win, font, newSize)
  ...
  (* redraw damaged lines (but not the cursor) *)
    fun redrawText (TXT{size, txt_buf, txt_win, ...}, damage) = let
	  ...
	  fun markDamage [] = ()
		...
		val topLn = (y - pad) quot char_ht
		val botLn = min(((y - pad) + ht + (char_ht-1)) quot char_ht, rows)
		val minC = (x - pad) quot char_wid
		val maxC = min(((x - pad) + wid + (char_wid-1)) quot char_wid, cols)
	  ...
	  in
(* CIO.print "redraw start\n"; *)
	    markDamage damage;
	    (redrawDamagedLines 0) handle _ => ()
(* ;CIO.print "redraw done\n" *)
	  end
...
(* CIO.print "resize start\n"; *)
   ...
   *)
    ...
    datatype req_msg
      ...
      | WriteStr of {where : char_coord, str : string}
      | HighlightStr of {where : char_coord, str : string}
      | InsertText of {where : char_coord, str : string, highlight : bool}
      | DeleteChars of {where : char_coord, cnt : int}
  ...
  (* create a new text widget *)
    fun mkTextWidget root {rows : int, cols : int} = let
	  ...
	  fun realize {env, win, sz} = let
		...
		val text = TXT{
			...
			txt_win = mkTextWin (win, font, tsz),
			cursor = {is_on = false, pos=ChrCrd{row=0, col=0}}
		      }
		fun server text = let
			...
			| handleReq (ClearScr cc) = (
			    clearEOS (text, cc);
			    server text)
			| handleReq (HighlightStr{where, str}) = (
			    highlightStr (text, where, str);
			    server text)
			| handleReq (WriteStr{where, str}) = (
			    writeStr (text, where, str);
			    server text)
			| handleReq (InsertText{where, str, highlight}) = (
			    insertText (text, where, str, highlight);
			    server text)
			| handleReq (DeleteChars{where, cnt}) = (
			    deleteChars (text, where, cnt);
			    server text)
	  ...
	  in
		...
		query = (fn req => (CML.send(reqCh, req); CML.accept replyCh)),
    ...
    fun ptToCoord tw pt = let
	  ...
	  in
	    ChrCrd{
	        row = (y - pad) quot char_ht,
		col = (x - pad) quot char_wid
	      }
	  end
    ...
    fun deleteChars (TW{cmd, ...}) {at : char_coord, cnt : int} =
          if cnt > 0 then cmd (DeleteChars{where=at, cnt = cnt})
          else ()
