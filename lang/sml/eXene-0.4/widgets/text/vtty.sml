  ...
  struct
    ...
    local
      ...
      fun expandTab col = let
	    ...
	    in
	      expand (tabStop - (col rem tabStop))
	    end
      ...
      fun mkEchoServer (keyEvt, trans, drawCh, putData) = let
	    ...
	    fun loop curLine = (case (msgBodyOf (sync keyEvt))
		 of (KEY_Press arg) => let
		      fun tab () = (send (drawCh, Draw "\t"); loop ("\t" :: curLine))
		      fun newLine () = (
			    send (drawCh, Draw "\n");
			    putData (implode (rev ("\n"::curLine)));
			    loop [])
	...
	| MapText of {nlines : int, ln_wid : int}
      ...
      fun mkHistoryBuf len = let
	    ...
	    fun config (maxLen, initRear) = let
		  ...
		  fun server (n, front, rear) = (case (accept reqCh)
     ...
     *)
      fun mkDrawServer (tw, getDataEvt, echoCh, cmdInEvt, twCmdCh) = let
	    ...
	    val echoEvt = receive echoCh
	    fun fillText l = let val row = ref 0
				 val _ = clear()
			     in app (fn s => (write(!row,0,s); inc row)) l
			     end
...
***)
		 ...
		 *)
		  fun server (arg as (cursorR, cursorC, curLnLen, curLn)) = let
		        open Interact
			fun handleOutput s = server (revfold
			       (fn (s,(row,col,len,ln)) => case s of
				"\^M" => (pushLine(hb,implode (rev ln));
					  (row+1,0,len,[]))
			      | "\n" =>  (pushLine(hb,implode (rev ln));
					  (row+1,0,len,[]))
			      | _ => (write (row,col,s); (row,col+1,len,s::ln)))
			      (explode s) arg)
    ...
    fun mkVtty root size = let
	  ...
	  val (putData, instrm) = CIO.openChanIn()
	  val (getDataEvt, outstrm) = CIO.openChanOut()
	(* realize the vtty. *)
