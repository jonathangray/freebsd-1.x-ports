...
structure Box : BOX =
  struct
    ...
    fun makeCO cl = let
          ...
          fun f ((_,_,repl),l) = revfold f' repl l
          in
            CML.choose(revfold f cl [])
          end
    ...
    fun mkLayout root (w as WBox _) = mkLayout root (HzCenter [w])
      ...
      | mkLayout root boxes = let
          ...
          val getvis = cvtFn o (I.find (fn (_,(true,v,_)) => SOME v | (_,(false,_,_)) => NONE))
          ...
          fun realizeBox {env=inenv as InEnv{co=myco,...}, win, sz} ctree = let
                ...
                fun zombie (me : layout_rep) = let
                      ...
                      fun loop () =
                            loop(select [
                              wrap(receive rqstChan, handleReq),
                              wrap (myci, fn _ => ()),
                              wrap (childco, fn _ => ())
                            ])
                      in
                ...
                and main me = let
                      ...
                      fun loop me =
                            loop(select [
                              ...
                              wrap(receive rqstChan, fn msg => handleReq (me,msg))
                            ])
                      in
                        loop me
                      end
          ...
          in
            ...
            Box {
              widget =mkWidget{
                  root = root,
                  boundsOf = (fn () => (send (rqstChan, GetSize); accept sizeChan)),
