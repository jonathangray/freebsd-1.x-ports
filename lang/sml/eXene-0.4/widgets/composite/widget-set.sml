...
structure WidgetSet : WIDGET_SET = 
  struct
    ...
    datatype item_msg = 
        Remove 
      | Pick of unit CML.cond_var
      | Unpick of unit CML.cond_var
    ...
    fun mkRep (state,pfn,afn,wchan) = 
          IR {state = ref state,pick_fn = pfn, 
               active_fn = afn, evt = CML.receive wchan}

    fun wrapW (w, wchan) = let
          open CML Widget Interact
          fun realize {env=env as InEnv{m,ci,k,...},win,sz} = let
                val mchan = channel() and cchan = channel() and kchan = channel()
                val env' = 
                  replaceKey(replaceCI(replaceMouse(env,receive mchan),receive cchan),receive kchan)

                fun check wf = let
                      val cv = CML.condVar ()
                      in send(wchan, wf cv); CML.readVar cv end
          ...
          in
            mkWidget{root = rootOf w, boundsOf = boundsFn w, realize = realize}
          end
    ...
    fun mkWidgetEvt slist = let
          ...
          in CML.choose(#2(revfold wf slist (0,[]))) end
    ...
    fun mkWidgetSet (pick,setPick,getPick) root (items : set_item list) = let
          open CML
          val (slist, wlist) = fold doItem items ([],[])
          ...
          fun main (picked, slist) = let
                ...
                fun handleRqst (SetChosen setl,picked) =
                      (((revfold picki setl picked) before (send(repChan, Okay)))
                  ...
                  | handleRqst (Insert ((index,ilist),rc), picked) =
                        case doInsert(picked, slist, index, ilist) of
                          Success(p,s,wl) => (send(rc,Widgets wl); main(p,s))
                ...
                fun loop picked =
                      loop(select[
                        wrap(receive rqstChan, fn rqst => handleRqst(rqst,picked)),
    ...
    fun setPick1 slist = let
          ...
          in #2(revfold setp slist (0,NONE)) end
    fun getPick1 (NONE,_) = []
    ...
    local
      ...
      fun command wrapfn (WS{rqstChan,repChan,...}) =
            fn arg => 
              ...
              case CML.accept repChan of Error e => raise e | Okay => ())
    in
    ...
    fun insert (WS{rqstChan,...}) arg = let
            ...
            in
              ...
              case CML.accept retc of Widgets wl => wl | IError e => raise e
            end
