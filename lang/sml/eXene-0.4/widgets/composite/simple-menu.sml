    ...
    fun layoutMenu (font, menu, label) = let
	  ...
	  fun menuGeom (MENU items, label) = let
		fun maxW (m, hasSubm, nitems, []) =
		  ...
		  | maxW (m, hasSubm, nitems, MenuItem(s, _) :: r) =
		      maxW(max(m, textWid s), hasSubm, nitems+1, r)
		  | maxW (m, hasSubm, nitems, Submenu(s, _) :: r) =
		      maxW(max(m, (textWid s) + iconWid + iconSp), true, nitems+1, r)
		in
		  case label
	  ...
	  fun layout (menu as (MENU items), label) = let
		val (maxWid, itemHt, totHt) = let
		      ...
		      val itemHt = 
                        if hasSubm then max(fonth+vspace, iconHt) else fonth+vspace
		      in
   ...
   *)
    fun createMenuWin (scr, submenuIcon, mrep, pos as PT{x=menuX, y=menuY}) = let
	  ...
	  fun selectItem (pt as PT{x, y}) = if within(pt, itemsRect)
		then SOME(nth(items, (y - itemsY) quot item_ht))
		else NONE
	  in
	    case label
...
(** NOTE: the "'a menu_rep" constraint is because of a bug in the typechecker **)
    fun popMenu (menuRep : 'a menu_rep, mbut, scr, icon, menupt, where, mouse) = let
	...
	(* adjust the position of a menu, to insure that it will fit on the screen *)
	  fun clipMenu (PT{x, y}, MREP{sz=SIZE{wid, ht}, ...}) = PT {
		  x = max(bwid, if (x+wid < scrWid-bwid) then x else (scrWid - (wid+bwid))),
		  y = max(bwid, if (y+ht < scrHt-bwid) then y else (scrHt - (ht+bwid)))
		}
	  ...
	  in
	    #1(popup (menuRep, menupt, where, fn _ => false))
	  end (* popMenu *)
  ...
  (* return true if the menu has a sub-menu *)
    ...
    fun attach (selCh, widget, mbuts, menu, label, where) = let
	  open CML
	  ...
	  fun realize {env as InEnv{m, ...}, win, sz} = let
		...
		fun loop () = let
			...
			case (msgBodyOf msg)
			 of (MOUSE_FirstDown(arg as {scr_pt, but, ...})) => if mbutIsSet(menuMBS, but)
			      then (
				case (popMenu (menuRep, but, scr, icon, where (WI arg), scr_pt, m))
				 of NONE => ()
		...
		in
		  spawn loop;
		  W.realizeFn widget 
                    {env = replaceMouse(env, receive mCh), win=win, sz=sz};
		  ()
		end (* realize *)
	  in
	    mkWidget{root=root, boundsOf=boundsFn widget, realize=realize}
	  end (* attach *)

    fun attachMenu (widget, mbuts, menu) = let
	  val selCh = CML.channel()
	  in
	    (attach(selCh, widget, mbuts, menu, NONE, fn _ => Item 0), CML.receive selCh)
	  end

    fun attachLabeledMenu (widget, mbuts, label, menu) = let
	  val selCh = CML.channel()
	  in
	    (attach(selCh, widget, mbuts, menu, SOME label, fn _ => Item 0), CML.receive selCh)
	  end

    fun buttonMenu (widget, mbuts, menu, where) = let
	  val selCh = CML.channel()
	  in
	    (attach(selCh, widget, mbuts, menu, NONE, where), CML.receive selCh)
	  end
