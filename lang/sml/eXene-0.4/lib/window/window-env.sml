...
(** NOTE: we may also want the modifier-key state **)
  ...
  (* createWinEnv : unit -> (in_env * out_env) *)
    fun createWinEnv () = let
	  ...
	  fun outEvt ch x = transmit(ch, x)
	  in
	    (InEnv{
		m = receive mCh, k = receive kCh, ci = receive ciCh, co = outEvt coCh
	      },
	     OutEnv{
		m = outEvt mCh, k = outEvt kCh, ci = outEvt ciCh, co = receive coCh
	      })
	  end
    ...
    fun addrLookup map = let
	  val lookup = HashWindow.findWinId map
	  in
   ...
   *)
    local
      val ignoreEvt = wrap (always (), fn () => raise SyncOnIgnoredStream)
      fun ignore evt = let
  ...
  (* an input stream that never produces messages *)
    val nullStream : 'a addr_msg event = choose[]
