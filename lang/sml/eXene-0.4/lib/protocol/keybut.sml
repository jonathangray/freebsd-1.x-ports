...
structure KeyBut : KEY_BUT =
  ...
  (** Modifier key states **)
    ...
    fun mkModState l = let
	  ...
	  in
	    (f (l, 0)) handle Any => AnyModKey
	  end

    fun shiftIsSet AnyModKey = true
      | shiftIsSet (MKState s) = ((s & shiftMask) <> 0)
    fun lockIsSet AnyModKey = true
      | lockIsSet (MKState s) = ((s & lockMask) <> 0)
    fun cntrlIsSet AnyModKey = true
      | cntrlIsSet (MKState s) = ((s & cntlMask) <> 0)
    fun modIsSet (AnyModKey, _) = true
      | modIsSet (MKState s, i) = ((s & (mod1Mask << (i-1))) <> 0)
  ...
  (** Mouse button states **)
    ...
    fun invertMBut (MBState s, MButton b) =
	  MBState(Bits.xorb(s, (but1Mask << (b-1))))

    fun mkButState l = let
	  fun f ([], m) = MBState m
	    | f ((MButton i)::r, m) = f (r, m ++ (but1Mask << (i-1)))
	  in
	    f(l, 0)
	  end

    fun mbutAllClr (MBState s)  = ((s & allButMask) = 0)
    fun mbutSomeSet (MBState s) = ((s & allButMask) <> 0)
    fun mbut1IsSet (MBState s)  = ((s & but1Mask) <> 0)
    fun mbut2IsSet (MBState s)  = ((s & but2Mask) <> 0)
    fun mbut3IsSet (MBState s)  = ((s & but3Mask) <> 0)
    fun mbut4IsSet (MBState s)  = ((s & but4Mask) <> 0)
    fun mbut5IsSet (MBState s)  = ((s & but5Mask) <> 0)
    fun mbutIsSet (MBState s, MButton i) = ((s & (but1Mask << (i-1))) <> 0)
