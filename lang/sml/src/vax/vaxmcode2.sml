...
fun outofrange s = raise(BadReal s)
...
fun emitarg (direct(reg r)) = regmode(5,r)
  ...
  | emitarg (immed i) = 
	   (emitarg(autoinc pc); emitlong i)
  | emitarg (displace(i,reg r)) =
		  (regmode(14,r); emitlong i)
  | emitarg (deferred(i,reg r)) =
	   (regmode(15,r); emitlong i)
...
fun args23(f2,f3) (args as (a,b,c)) = f3 args

fun immedbyte(i) =
	     (emitarg(autoinc pc); signedbyte i);

fun immedword(i) =
	     (emitarg(autoinc pc); emitword i);

fun emitlab (i,lab) = jump(LABPTR i, lab)

fun jbr (address lab) = jump(JBR,lab)
fun bbc (arg1, arg2, address lab) =
	    let val r = (ref 0, 14*16+1,14*16+0)
	     in jump(WHICH r, lab); emitarg arg1; emitarg arg2; jump(COND r, lab)
	    end
fun bbs (arg1, arg2, address lab) =
	    let val r = (ref 0, 14*16+0,14*16+1)
	     in jump(WHICH r, lab); emitarg arg1; emitarg arg2; jump(COND r, lab)
	    end

fun movb args = (emitbyte (9*16); emit2arg args)

fun movzbl args = (emitbyte (9*16+10); emit2arg args)

fun pushal args = (emitbyte (13*16+15); emitarg args)

fun addl2 args = (emitbyte (12*16); emit2arg args)

fun moval args = (emitbyte (13*16+14); emit2arg args)

fun movl args = (emitbyte (13*16); emit2arg args)
...
fun ashl args = (emitbyte (7*16+8); emit3arg args)
