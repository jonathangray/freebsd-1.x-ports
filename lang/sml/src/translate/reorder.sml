 ...
 *  Sethi-Ullman reordering of expression trees to minimize register usage
    ...
    to compute   let val n=N and m=M in (M,N) end
    instead.
...
datatype info = I of {exp:lexp,  (* The expression itself *)
...
fun cost((I{regs,...},_)::rest) = max(regs, 1+cost rest)
  | cost nil = 0
...
fun combine(l,do_it) =
 let fun g(I{exp=e1,side=s1,regs=r1,fetch=f1,alloc=a1}::rest, e,s,r,f,a) =
            g(rest, e1::e, s1 orelse s, max(1+r, r1), f1 orelse f, a1 orelse a)
...
fun fetchprim P.BOXED = true
  ...
  | fetchprim P.FSUBSCRIPTd = true
  | fetchprim P.INLFSUBSCRIPTd = true
...
fun sort(do_it, l) = if inorder l 
     then combine(l, do_it)
		...
		| rename (I{exp,...},NONE) = exp
	      ...
	      fun bind ((_,NONE), e) = e
		| bind ((I{exp,...},SOME v),e) = APP(FN(v,BOGUSty,e),exp)

	   in I{regs= cost l'',
	        side = anyside l'',
	        fetch = anyfetch l'',
	        alloc = anyalloc l'',
	        exp = fold bind l'' (do_it (map rename l'))
	       }
	  end

val many = 12   (* how many regs to charge a function call *)

val rec reorder : lexp -> info =
  fn e as VAR _ => I{regs=0, side=false, exp=e, fetch=false, alloc=false}
   | FN(v,t,e) => I{regs=1, side=false, fetch=false, alloc=false,
		    exp= FN(v,t,getexp(reorder e))}
   | FIX(vl,t,el,e) => let val I{regs,side,exp,fetch,alloc} = reorder e 
                        in I{regs=regs+1,side=side,fetch=fetch,alloc=alloc,
		   	     exp=FIX(vl,t,el,exp)}
                       end
   | APP(p as PRIM(i,t), b) => 
          let val I{regs,side,fetch,alloc,exp=e1} = reorder b
	   in I{regs=max(1,regs),side=not(Prim.pure i), 
	        alloc=false, fetch=fetchprim i, exp=APP(p, e1)}
	  end
   | APP(FN(v,t,a),b) => 
	  let val I{regs=ra,side=sa,exp=ea,fetch=fa,alloc=aa} =reorder a
	      val I{regs=rb,side=sb,exp=eb,fetch=fb,alloc=ab} =reorder b
	   in I{regs=max(rb,1+ra),side=sa orelse sb,
	        fetch= fa orelse fb, alloc=aa orelse ab,
	        exp=APP(FN(v,t,ea),eb)}
	  end
   | APP(a,b) => 
          let val I{exp=e1,...} = 
   ...
   | e as INT i => I{regs=0, side=false, fetch=false, alloc=false, exp=e}
   | e as REAL i => I{regs=0, side=false, fetch=false, alloc=false, exp=e}
   | e as STRING i => I{regs=0, side=false, fetch=false, alloc=false, exp=e}
   | SWITCH(e0,sign,l,d) => 
         let val I{regs,side,exp,fetch,alloc}= reorder e0
	     fun combine((c,e),(r,s,f,a,el)) =
		    let val I{regs,side,exp,fetch,alloc}=reorder e
		     in (max(r,regs),s orelse side,f orelse fetch,
			a orelse alloc,(c,exp)::el)
		    end
             val (lr,ls,lf,la,l') = fold combine l (regs,side,fetch,alloc,nil)
   ...
   | CON(c,e) => let val I{regs,side,exp,fetch,alloc} = reorder e
	          in I{regs=max(1,regs), side=side, fetch=fetch, alloc=alloc,
		       exp=CON(c,exp)} (* close enuf *)
		 end
   | DECON(c,e) => let val I{regs,side,exp,fetch,alloc} = reorder e
	          in I{regs=max(regs,1), side=side, fetch=true,alloc=alloc,
		       exp=DECON(c,exp)}
		 end
   | RECORD l => sort(fn x => RECORD x, map reorder l)
   | VECTOR l => sort(fn x => VECTOR x, map reorder l)
   | SELECT(i,e) => let val I{regs,side,exp,fetch,alloc} = reorder e
                     in I{regs=max(1,regs),side=side,fetch=true,alloc=alloc,
			  exp=SELECT(i,exp)}
                    end
   | RAISE(e,t) =>  let val I{regs,side,exp,fetch,alloc} = reorder e
                     in I{regs=max(1,regs),side=true,fetch=fetch,alloc=alloc,
			  exp=RAISE(exp,t)}
                    end
   ...
   | WRAP(t,e) => let val I{regs,side,exp,fetch,alloc} = reorder e
                   in I{regs=regs, side=side, fetch=true, alloc=alloc,
                        exp=WRAP(t,exp)}
                  end 
   | UNWRAP(t,e) => let val I{regs,side,exp,fetch,alloc} = reorder e
                     in I{regs=regs, side=side, fetch=true, alloc=alloc,
                          exp=UNWRAP(t,exp)}
                    end 
   | e as PRIM(i,t) => I{regs=0,side=false,fetch=false,alloc=false,exp=e}

val reorder = fn x => getexp (reorder x)
