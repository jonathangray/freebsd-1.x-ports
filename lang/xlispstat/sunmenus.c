...
#undef SUNVIEW /* to remofe a conflict in the include files */
...
IVIEW_MENU get_hardware_menu(menu)
	LVAL menu;
{
          ...
          caddr_t arg;
{
  if (event_is_down(event) && event_id(event) == MS_RIGHT) {
    ...
    window_set(win, WIN_EVENT_PROC, nil, 0);
    window_return(0);
  }
}
	...
	int x, y;
{
  ...
  if (mouse_event != nil && mouse_window != nil) {
