...
static draw_choice(dpy, win, item)
     Display *dpy;
     Window win;
     LVAL item;
{
     ...
     LVAL item;
{
  ...
  if (XSaveContext(dpy, choice, ObjectContext, (XContext) item) != 0)
    xlfail("could not install object in window");
}

DeleteChoiceItem(win, item)
     Window win;
     LVAL item;
{
  Display *dpy = StX11Display();
     ...
     int *width, *height;
{
  ...
  if (width != nil) *width = i_sz.h + CHOICE_PAD;
  if (height != nil) *height = i_sz.v;
}
