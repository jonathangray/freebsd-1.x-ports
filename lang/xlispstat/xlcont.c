...
LVAL xsetq()
{
    ...
    /* check for a constant - L. Tierney */
    if (isconstant(sym)) xlfail("can't assign to a constant");
...
LVAL xpsetq()
{
    ...
    for (; plist; plist = cdr(plist)) {
      /* check for a constant - L. Tierney */
      if (isconstant(car(plist))) xlfail("can't assign to a constant");
...
LVAL xsetf()
{
	...
	if (symbolp(place)) {
      /* check for a constant - L. Tierney */
      if (isconstant(place)) xlfail("can't assign to a constant");
  ...
  LVAL place,value;
{
	...
	arg1 = evmatch(SYMBOL,&place);
  /* check for a constant - L. Tierney */
  if (isconstant(arg1)) xlfail("can't assign to a constant");
	if (place) toomany(place);
	setvalue(arg1,value);
    }
	...
	xlpop();
#else
	evsetarrayelement(place, value); /* L. Tierney */
#endif /* XLISP_ONLY */
    }
#ifndef XLISP_ONLY
    else if (fun == xlenter("ELT")) { /* L. Tierney */
      arg1 = evarg(&place);
      arg2 = evmatch(FIXNUM, &place);
      set_elt(arg1, arg2, value);
    }
#endif /* XLISP_ONLY */
...
LVAL xdefun()
{
  ...
  if (consp(arglist) && stringp(car(arglist)) && consp(cdr(arglist))) {
    set_function_docstring(sym, car(arglist));
    arglist = cdr(arglist);
  }
...
LVAL evmatch(type,pargs) /* changed, Luke Tierney, 3 March, 1988 */
  int type; LVAL *pargs;
{
    LVAL arg;
