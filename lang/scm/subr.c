...
SCM n,cnt;
{
  ...
  if(cnt < 0) {
    ...
    if NFALSEP(negativep(n))
      return sum(MAKINUM(-1),lquotient(sum(MAKINUM(1),n),res));
...
SCM n;
{
  ...
  if NINUMP(n) {
    ...
    if BIGSIGN(n) return logcount(difference(MAKINUM(-1),n));
    ds = BDIGITS(n);
    for(i = NUMDIGS(n); i--; )
...
SCM n;
{
  ...
  if NINUMP(n) {
    ...
    if BIGSIGN(n) return intlength(difference(MAKINUM(-1),n));
    ds = BDIGITS(n);
    d = ds[c = NUMDIGS(n)-1];
