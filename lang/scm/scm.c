...
static void init_banner()
{
  ...
  fputs(PATCHLEVEL,stderr);
  fputs(", Copyright (C) 1990, 1991, 1992, 1993 Aubrey Jaffer.\n\
...
int run_scm(iverbose,argc,argv)
int iverbose;
int argc;
char **argv;
{
...
# endif
#endif
  return run_scm((isatty(fileno(stdin)) && isatty(fileno(stdout)))
		 ? (argc <= 1) ? 2 : 1 : 0, argc, argv );
}
#endif
