...
int minr, minc, maxr, maxc;
{
    ...
    if (p && p->label) {
	pr = Malloc((unsigned)(strlen(p->label)+1));
	Strcpy(pr, p->label);
	if (p->cellerror)
...
VOID_OR_INT eval_fpe() /* Trap for FPE errors in eval */
{
 ...
 * Take string arguments which they Free.
...
register char *s1, *s2;
{
    ...
    arg1 = s1 ? s1 : "";
    arg2 = s2 ? s2 : "";
    p = Malloc((unsigned)(strlen(arg1)+strlen(arg2)+1));
    Strcpy(p, arg1);
    Strcat(p, arg2);
    if (s1)
        Free(s1);
...
double v;
{
    ...
    p = Malloc((unsigned)(strlen(buff)+1));
    Strcpy(p, buff);
    Free(fmtstr);
    return(p);
}
...
double value;
{
    ...
    if (command)
	Free(command);
    return (strcpy (Malloc((unsigned) 1), "\0"));
}
...
double value;
{
	...
	if ((! command) || (! *command)) {
		...
		if (fgets (buff, sizeof(buff)-1, pp) == NULL)	/* one line */
			...
			*cp = '\0';			/* end string there */
			...
			prevstr = Realloc(prevstr, prevlen);
		    }
		    Strcpy (prevstr, buff);
			 /* save alloc'd copy */
		}
    ...
    if (prevstr)
	return (strcpy (Malloc ((unsigned) (strlen (prevstr) + 1)), prevstr));
    else
	return (strcpy(Malloc((unsigned)1), ""));
}
    ...
    double rowdoub;
{
    ...
    return (strcpy (Malloc ((unsigned) (strlen (llabel) + 1)), llabel));
}
...
register struct enode *se;
{
    ...
    switch (se->op) {
	case O_SCONST: p = Malloc((unsigned)(strlen(se->e.s)+1));
		     Strcpy(p, se->e.s);
		     return(p);
	case O_VAR:    {
			struct ent *ep;
			...
			if (!ep->label)
			    return((char *)0);
			p = Malloc((unsigned)(strlen(ep->label)+1));
			Strcpy(p, ep->label);
			return(p);
		     }
	...
	case COLTOA: return(strcpy(Malloc((unsigned)10),
...
struct enode *a1, *a2;
{
	...
	p = (struct enode *) Malloc ((unsigned)sizeof (struct enode));
    p->op = op;
    p->e.o.left = a1;
    p->e.o.right = a2;
    return p;
...
char *s;
{
	...
	for (c = minc; c <= maxc; c++) {
	    ...
	    n->format = 0;
	    if (s && *s != '\0')
		n->format = strcpy(Malloc((unsigned)(strlen(s)+1)), s);
	    n->flags |= is_changed;
       }
}
