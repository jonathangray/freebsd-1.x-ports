...

static char rx_version_string[] = "GNU Rx version 0.01";
...
/* This page is to the interesting subsystems and lower layers
  ...
  for (future = state->futures; future; future = future->next)
    {
      ...
      for (dfp = df; dfp; dfp = dfp->next_same_super_edge)
	      ...
	      if (!zero_times_ok)
		{
		  struct rexp_node * concat
		    = rx_mk_r_concat (&rxb->rx, inner_exp,
				      rx_copy_rexp (&rxb->rx, *last_expression));
		  if (!concat)
		    return REG_ESPACE;
 ...
 */
   ...
   */
	...
	(&rxb->rx,
	 ...
	 (&rxb->rx,
	  ...
	  (&rxb->rx,
	   rx_mk_r_concat
	   (&rxb->rx,
	    rx_mk_r_side_effect (&rxb->rx, (void*)re_se_try),
	    fewer_side_effects),
	   rx_mk_r_side_effect (&rxb->rx,(void*)re_se_win))),
	 rx_mk_r_concat
	 (&rxb->rx,
	  rx_mk_r_cset (&rxb->rx, cs2),
	  rx_mk_r_concat
	  (&rxb->rx,
	   rx_mk_r_concat
	   (&rxb->rx,
    ...
    if (!(new_rexp && cs && cs2))
      return REG_ESPACE;
    RX_bitset_enjoin (cs2, 'm'); /* prefixed to the rexp used for matching. */
    RX_bitset_enjoin (cs, 's'); /* prefixed to the rexp used for searching. */
    rexp = new_rexp;
  }
...
#define PUSH(CHUNK_VAR,BYTES)   \
  if (!CHUNK_VAR || (CHUNK_VAR->bytes_left < (BYTES)))  \
	...
	{				\
      ...
      new_chunk->bytes_left = chunk_bytes - (BYTES); \
...

   ...
   */
  {
    ...
    /* Two nfa's were compiled.  `m' is complete, `s' faster. */
    int nfa_choice = ((regs && !rxb->least_subs) ? 'm' : 's');
	  ...
	  while (1)
			...
			if (effect < 0)
			  {
				 ...
				 */
				{
				  ...
				  if (best_last_r < 0)
				    new_best = 1;
				  else if (urhere + 1 < best_rparen[0])
				    new_best = 0;
				  else if (urhere + 1 > best_rparen[0])
				    new_best = 1;
				  else if (urhere + 1 == best_rparen[0])
		...
		case OF(rx_backtrack_point,backtrack_point):
		  ...
		  test_do_return:
		    ...
		    bf->df = bf->df->next_same_super_edge;
		    super = bf->stk_super;
