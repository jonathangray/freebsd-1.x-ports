 ...
 *		$Revision: 1.1 $
 *
 *              Some Mod's by Art Mulder, 1992.
 *                Look for "CHANGE/OLD CODE/NEW CODE" comments
 */
...
/*
** CHANGE:
**  Under `sc' a call to 'erase' with no argument, would erase from
**  (showsr,showsc) to the current row,column.  This would erase from
**  wherever the last tab (range) region started, to the current
**  row,column.  I think this is wrong.  A call to erase with no
**  argument should erase the cell at the current row,column position
**  only.
**
** OLD CODE:
**	|	S_ERASE       
...
/*
** CHANGE: the `value' command has the same bug as the `erase'
** command, with regards to the action taken when no argument is
** present.  The same fix is applied.
**
** OLD CODE:
...
** NEW:
*/
	|	S_VALUE       { valueize_area(currow, curcol, currow, curcol);
				 modflg++; }
...
/*
** CHANGE: the `fill' command have a similar bug to the `erase'
** command, w.r.t. the action taken when no range argument is present.
**
** OLD CODE:
**
...
** NEW:
*/
	|	S_FILL num num  { fill(lookat(currow, curcol),
				      lookat(currow, curcol), $2, $3); }
...
/*
** CHANGE: the `lock' and `unlock' commands have the same bug as the
** `erase' command, w.r.t. the action taken when no argument is present.
**
** OLD CODE:
**	|	S_LOCK       
...
*/
	|	S_LOCK       
					{ lock_cells(lookat(currow, curcol),
				        lookat(currow, curcol)); }
	|	S_LOCK var_or_range
...
*/
	|	S_UNLOCK       
					{ unlock_cells(lookat(currow,curcol),
				        lookat(currow, curcol)); }
...
/*
** CHANGE:
**  Under `sc' a call to 'define' with no argument, would define a name from
**  (showsr,showsc) to the current row,column.  This would define a name from
**  wherever the last tab (range) region started, to the current
**  row,column.  This doesn't work with `ss'.  A call to define with no
**  argument should define a name for the cell at the current row,column
**  position only.
**  
** OLD CODE:
...
** NEW:
*/
	|	S_DEFINE strarg       { struct ent_ptr arg1;
					arg1.vp = lookat(currow, curcol);
					arg1.vf = 0;
					add_range($2, arg1, arg1, 0);
				      }
