 ...
 *----------------------------------------------------------------------
 */
static int hold_flag = FALSE;	/* Flag: are we holding a character? */
static int hold_c = 0; 		/* Character being held */

int nmungetch(c)
/*----------------------------------------------------------------------
** Place c back onto the input queue to  be  returned  by
** the next call to nmgetch().
**
** In various places throughout the code we make use of the System V
** curses function "ungetch()" [ similar to the standard library
** function ungetc() ].  Unfortunately some systems do not have 
** that function, so we fake it here by having our own internal
** one character buffer.
**
** NOTE: this requires that all nmgetch() functions first check this
** buffer before reading the real input stream.
*/
    register int   c;	/* character to be pushed back on inp. stream */
{
#if defined(NOUNGETCH)
   hold_c = c;
   hold_flag = TRUE;
#else
   ungetch(c);
...
/*
 * a)	Input Routines [ nmgetch() ], for "SIMPLE" version.
 *----------------------------------------------------------------------
 */
#ifdef SIMPLE
...
/*
 * Non-VMS "SIMPLE" input routine 
 */
int nmgetch()
{
    Refresh(); 	/* refresh screen before getting input */

/** This section of code required by above nmungetch() function **/
    if (hold_flag == TRUE) {	/* check if we are holding a char */
      hold_flag = FALSE;
      c = hold_c;
      return (toascii(hold_c));
    } else {
...

/*
 * b)	Input Routines for Some Regular (Non-SIMPLE) versions. 
 *----------------------------------------------------------------------
 */
...

/*
 * c)	Input Routines for Other Regular (Non-SIMPLE) versions. 
 *----------------------------------------------------------------------
 */

#if defined(SYSV2) || defined(SYSV3) || defined(ULTRIX) || defined(MSDOS)
/** #if defined(SYSV2) || defined(SYSV3) || defined(MSDOS) **/

/* 
 * The notimeout() function (see the curses man page) is useful on
 * slower systems.  
 */
void
initkbd()
...
/*
 * Non-SystemV curses don't have the KEY_* definitions for function
 * keys.  So, here we convert all KEY_* to something else.
 * Therefore SystemV function key dependence only is found here.
 *
 * Doing it here raises the problem of KEY_* macros which convert to
 * to multi-key command combinations (ie: ESC-v).  ungetch() is
 * the way around that,.
 */

		/* Unix SystemV R3 does a refresh in getch */ 
...
/*
 * if KEY_LEFT is defined, then we assume that *all* System-V Curses
 * function key names are defined also.
 */
#ifdef KEY_LEFT
      case KEY_LEFT:  	   c = kLEFT; 	break;
      case KEY_RIGHT: 	   c = kRIGHT; 	break;
      case KEY_UP:    	   c = kUP; 	break;
      case KEY_DOWN:  	   c = kDOWN; 	break;
      case KEY_BACKSPACE:  c = kBS; 	break;
      case KEY_NPAGE:	   c = kPGDN;	break;
  
   /* With the two key combinations we have to get tricky ... */
      case KEY_PPAGE:  c = kMETA;  nmungetch(kPGUP);    break;
      case KEY_HOME:   c = kMETA;  nmungetch(kHOME);    break;
# ifndef ULTRIX
/* /usr/include/cursesX.h is missing these in Ultrix */
      case KEY_END:    c = kMETA;  nmungetch(kEND);     break;
      case KEY_SLEFT:  c = kCTRLX; nmungetch(kPGLEFT);  break;
      case KEY_SRIGHT: c = kCTRLX; nmungetch(kPGRIGHT); break;
# endif
#endif  /* #ifdef KEY_LEFT */
...
#endif /* SYSV2 || SYSV3 || ULTRIX || MSDOS */
