...
{
  struct rx_distinct_future * next_same_super_edge;
...
extern void regfree ();
   ...
   *
   * The ordering of these effects is significant when searching.  re_se_win
   * should always remain the most negative side effect.  The other values
   * are less critical but the goal is that the shorter path to a win
   * should always be the more negative.
   */
  ...
  /* Simple effects: */
  RX_DEF_SE(1, re_se_fail, = re_se_dollar - 1) /* These are rare, = so order doesn't matter. */
  RX_DEF_SE(1, re_se_win, = re_se_fail - 1)			/* Epsilon to a final state */

  /* Complex effects.  These are used in the 'se' field of 
   * an RX_DEF_SE(re_se_params structure.  Indexes into the se array
   * are stored as instructions on nfa edges.
   *
   * Don't be confused that these are declared here.  They never occur in the
   * op field of a struct rx_inx.
   */
  RX_DEF_CPLX_SE(1, re_se_lparen, = 0)
