...
/**     case (ctl('h')):                linelim = back_line();  break; **/
        case (ctl('h')): case DEL:      linelim = back_line();  break;
               /** treat DEL like Backspace **/
  ...
  /**   case (ctl('h')):                back_space();   break; **/
        case (ctl('h')): case DEL:      back_space();   break;
	case (ctl('m')):  cr_line();			break;
...
{   static	unsigned	undolen = 0;
	...
	undo_line = Realloc(undo_line, undolen);
    }
...
save_hist()
{
	...
	history[lasthist].histline = Realloc(history[lasthist].histline,
...
search_hist()
{
	...
	last_search = Realloc(last_search, lastsrchlen);
    }
