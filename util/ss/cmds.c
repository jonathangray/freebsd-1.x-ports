 ...
 *		$Revision: 1.1 $
 */
 /* patched in some of the sc Rev 6.21 stuff.  Art Mulder, 5/92 */
...
void deleterow(startrow,endrow)
/*----------------------------------------------------------------------
** Delete the rows from 'startrow' to 'endrow'.  (startrow must be
** <= endrow.)
*/
    register int startrow,endrow;
{
    if (any_locked_cells(startrow, 0, endrow, maxcol))
	error("Locked cells encountered. Nothing changed");
    else {
    	flush_saved();
    	erase_area(startrow, 0, endrow, maxcol);
/*    	currow += arg;
...
/* 
** ** ** ** ** ** ** ** ** ** ** ** ** ** **
** Art Mulder Modified:  See "rowcolumn.c"
** ** ** ** ** ** ** ** ** ** ** ** ** ** **
**
** void
** colshow_op()
...
/* 
** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** **
** Art Mulder Modified:  No Longer Used.  Jan 25/93
** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** **
** int
** get_rcqual (ch)
**     int ch;
...
/*----------------------------------------------------------------------
** Delete the cols from 'startcol' to 'endcol'.  (startcol must be
** <= endcol.)
*/
    int startcol,endcol;
{
    int cs = startcol;				/** Old var's **/
    int numcol = (endcol - startcol + 1);	/** Old var's **/
...
/* Check for errors */
    if (endcol > maxcol) {
	error("Ending Column exceeds Max column, No Change.");
	return;
    }
    if (any_locked_cells(0, startcol, maxrow, endcol)) {
	error("Locked cells encountered. Nothing changed");
	return;
    }
...
/* Clear the columns */
    flush_saved();
    erase_area(0, startcol, maxrow, endcol);
    sync_refs();
...
/** NOT sure how this works **/
	pp = ATBL(tbl, r, startcol);
	for (c=startcol; c <= lim; c++, pp++)
	{   if (c > lim)
		*pp = (struct ent *)0;
	    else
	    if (pp[0] = pp[numcol])
...
int r0, c0, rn, cn;
{
    ...
    if (!pline && (pline = Malloc((unsigned)(FBUFLEN *
	...
	for (pp = ATBL(tbl, row, col=c0); col<=cn;
		...
		if ((*pp)->flags&is_valid) {
		    while(plinelim + fwidth[col] > 
		      ...
		      if((pline = ((char *)Realloc((char *)pline, 
				    (unsigned)(FBUFLEN * ++fbufs_allocated))))
			 == NULL) {
			error ("Realloc failed in printfile()");
			return;
...
int r0, c0, rn, cn;
{
	...
	coldelim = '&';
      ...
      fprintf(f,
	"<MIFFile 3.00> # generated by the sc spreadsheet calculator\n ");
      fprintf(f,"<Tbls\n");
      fprintf(f," <Tbl \n");
      ...
      for (col=c0; col <= cn; col++) {
          ...
          fprintf(f,
	    "       <PgfTag `CellHeading'> # in Paragraph Format Catal og\n");
	...
	for (pp = ATBL(tbl, row, col=c0); col<=cn; col++, pp++) {
	      ...
	      fprintf(f,
		"       <PgfTag `CellBody'> # in Paragraph Format Cata log\n");
...
int Rdelta, Cdelta;
{
	...
	case 'f':
		...
		ret->e.o.left = (struct enode *)0;
 		break;
	case '$':
		ret->e.s = Malloc((unsigned) strlen(e->e.s)+1);
...
void hiderow(startrow,endrow)
/*----------------------------------------------------------------------
** Mark the rows from 'startrow' to 'endrow' (inclusive) as hidden.
*/
    register int startrow,endrow;
{
    currow = startrow;
    ...
    if (endrow >= maxrows-1) {
	if (!growtbl(GROWROW, (endrow - startrow +1), 0)) {
    ...
    modflg++;
    while (startrow <= endrow)
	row_hidden[startrow++] = 1;
}

void hidecol(startcol,endcol)
/*----------------------------------------------------------------------
** Mark the cols from 'startcol' to 'endcol' (inclusive) as hidden.
*/
    register int startcol,endcol;
{
    ...
    if (endcol >= maxcols-1) {
	if ((endcol >= ABSMAXCOLS) ||
	    	!growtbl(GROWCOL, 0, (endcol-startcol +1))) {
...
int *rpid;
{
	...
	*rpid = 0;
    ...
    deraw();
#ifdef VMS
    fprintf(stderr, "No child tasks available yet under VMS--sorry\n");
#else /* not VMS */
	    ...
	    int dr, dc;
{
    ...
    n -> format = 0;
    if (p -> format) {
        n -> format = Malloc((unsigned) (strlen (p -> format) + 1));
...
erasedb ()
{
	...
	for (c=0; c++<=maxcol; pp++)
		...
		if ((*pp)->label) Free ((char *)((*pp) -> label));
...
char	*path;
{
	...
	if (stat(path, &statbuf) == 0)
	{
		...
		{	buflen = statbuf.st_blksize;
			if ((buf = Realloc(buf, buflen)) == (char *)0)
			{	buflen = 0;
				return(0);
			}
		}
