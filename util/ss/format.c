  ...
  int buflen;
{
  ...
  {	fmtlen = strlen(fmt) + 40;
	tmpfmt1 = Realloc(tmpfmt1, fmtlen);
	tmpfmt2 = Realloc(tmpfmt2, fmtlen);
	exptmp = Realloc(exptmp, fmtlen);
  }
  fmt = strcpy(tmpfmt1, fmt);
  if (buflen + 1 > mantlen)
  {	mantlen = buflen + 40;
	mantissa = Realloc(mantissa, mantlen);
  }
   ...
   * format string stored in tmpfmt2 also Malloc()'d above
   */
  tmp = tmpfmt2;
 ...
 *   ce = strcpy(Malloc((unsigned)((len_ce = strlen(ce)) + 1)), ce);
 */
