...
#ifdef	RCSID
static char RCS[] =
	"$Header: /a/cvs/386BSD/ports/util/unshar/unshar.c,v 1.1 1993/09/04 16:45:34 jkh Exp $";
#endif	/* RCSID */
    ...
    REGISTER char	*av[];
{
    ...
    Saveit = DEF_SAVEIT;
    HdrFile = NULL;
    for (Forced = 0; (i = getopt(ac, av, "c:d:fh:sn")) != EOF; )
	switch (i) {
	default:
	    Quit("Usage: unshar [-fsn] [-{c,d} dir] [-h hdrfile] [files]");
	    /* NOTREACHED */
	case 'c': 
	case 'd': 
	    p = optarg;
