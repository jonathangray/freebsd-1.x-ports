 ...
 *	    Added swap statistics Wolfram Schneider <wosch@cs.tu-berlin.de>
 *	    Changed swap format Nate Williams <nate@bsd.coe.montana.edu>
 */
...

{
...
#else
        static int free, active, inactive, wired, swap;
	...
	memory_stats[8] = -1;
	memory_stats[9] = swap_used();
	if (memory_stats[9] < 0)  /* ignore errors */
	  memory_stats[9] = 0;
...
int swap_used()
{
	...
	if (kvm_read ((void *)nl [VM_NSWAP].n_value, &nswap, 
		      sizeof (nswap)) != sizeof (nswap)) 
	  return(-1);

	if (kvm_read ((void *)nl [VM_NSWDEV].n_value, &nswdev, 
		      sizeof (nswdev)) !=  sizeof (nswdev)) 
	  return(-1);

	if (kvm_read ((void *)nl [VM_DMMAX].n_value, &dmmax, 
		      sizeof (dmmax)) != sizeof (dmmax)) 
	  return(-1);


	if ((swdevt = (void *)malloc (sizeof (struct swdevt) * nswdev)) == 
	    NULL || 
	    (by_device = (void *)calloc (sizeof (*by_device), nswdev)) == 
	    NULL) 
	  return(-1);

	if (kvm_read ((void *)nl [VM_SWDEVT].n_value, swdevt, 
		      sizeof (struct swdevt) * nswdev) !=
	    sizeof (struct swdevt) * nswdev) 
	  return(-1);

	if (kvm_read ((void *)nl [0].n_value, &swapmap, 
	...
	total_free = 0;
		...
		if (kvm_read ((void *) swapmap, &head, sizeof (struct rlist )) 
		    != sizeof (struct rlist )) 
		  return(-1);
...
/*	header = getbsize("swapinfo", &headerlen, &blocksize); */
	...
	for (total_avail = total_partitions = i = 0; i < nswdev; i++) {
		...
		if (!swdevt [i].sw_freed) {
		  ...
		  total_partitions++;
		  total_avail += swdevt [i].sw_nblks/ (blocksize/512);
		  total_used += (swdevt [i].sw_nblks -
	...
	if ( total_avail == 0 )
		...
		return((int)100 * total_used/total_avail);
