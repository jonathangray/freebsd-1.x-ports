...
#define real_tty_output(x) (exp_stdin_is_tty && ((x)==1) || ((x)==exp_dev_tty))
#define real_tty_input(x) (exp_stdout_is_tty && ((x)==0) || ((x)==exp_dev_tty))

#define new(x)	(x *)ckalloc(sizeof(x))

struct action {
	char *statement;
	int tty_reset;		/* true, reset tty mode upon action */
...
CAN_MATCH, or CANT_MATCH.  These describe whether the keystrokes match
a key sequence, and could or can't if more characters arrive.  The
...
If there is a can-match and there are more keymaps to try, we continue
...
update_interact_fds(interp,fd_count,fd_to_input,fd_list,input_base,
			do_indirect,close_count,real_tty_caller)
Tcl_Interp *interp;
int *fd_count;
...
static int inter_indirect_key = 0;
...
/*		next_case_sensitive = TRUE;*/
	...
	indirect_key = inter_indirect_key;
	...
	for (;;) {
		...
		/* update the world, if:
		   indirects have been updated or
		   any fds have closed
		 */
		if (indirect_key != inter_indirect_key
		    || exp_close_count != close_count) {
			status = update_interact_fds(interp,&input_count,
					&fd_to_input,&fd_list,input_base,1,
					&close_count,&real_tty);
