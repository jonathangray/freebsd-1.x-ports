...
static exp_tty tty_current, tty_cooked;
...
/*		tty_current.c_lflag = tty_cooked.c_lflag;*/
		tty_current.c_lflag = tty_cooked.c_lflag & ~ECHO;
...
int fd;
{
#if defined(POSIX) && !defined(LYNX)
	tcsendbreak(fd,0);
#else
# ifdef TIOCSBRK
	ioctl(fd,TIOCSBRK,0);
	exp_usleep(interp,250000); /* sleep for at least a quarter of a second */
	ioctl(fd,TIOCCBRK,0);
# else
