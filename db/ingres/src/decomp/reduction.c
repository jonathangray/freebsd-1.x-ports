...
*/
int
algorithm(comp_list_t *clist, int varmap)
{
	register comp_list_t	*chead, *cnext;
	register int			var;
	int				vmap;
	comp_list_t			*cprev, *cl;
...
comp_list_t *
buildlist(qtree_t *root1, char *buf)
{
	register comp_list_t	*head, *next;
	register qtree_t			*root;
	comp_list_t			*ret;

	ret = (comp_list_t *) need(buf, 0);
	head = 0;

	for (root = root1; root->sym.type != QLEND; root = root->right) {
		next = (comp_list_t *) need(buf, sizeof(*next));
		next->clause = root;
...
qtree_t *
construct(qtree_t *root, comp_list_t *listhead, char *buf)
{
	register qtree_t			*ret, *q;
...
comp_list_t *
order(comp_list_t *clist, int ovlapvar)
{
	register comp_list_t	*cl, *joint, *disjoint;
	comp_list_t			*xd, *xj, *tlpiece, *ret;
	qtree_t				*tmp;
	int				map;

	xd = xj = (comp_list_t *) NULL;
