...
void
modqual(qtree_t *root, qtree_t **lnodv, int srcmap, int agvar)
{
	register qtree_t	*and_eq, *afcn;
	register int	i;
	register int	len;
	...
	for (i = 0; (afcn = lnodv[i]) != 0 ; i++) {
		...
		and_eq = (qtree_t *) need(De.de_qbuf, QT_HDR_SIZ + len);
		and_eq->sym.type = AND;
		and_eq->sym.len = len;
		...
		/* `EQ' node  */
		len = sizeof(opnode_t);
		and_eq->left = (qtree_t *) need(De.de_qbuf, QT_HDR_SIZ + len);
		and_eq = and_eq->left;
		and_eq->sym.type = BOP;
		and_eq->sym.len = len;
...
qtree_t *
byeval(qtree_t *root, qtree_t *aghead, int agvar)
/* root - root of orig query */
/* aghead - root of ag fcn sub-tree */
/* agvar - variable number assigned to this aggregate */
{

	register qtree_t 	*q, *ag, *resdom;
	qtree_t		*r;
	int		temp_relnum, i, filled;
	qtree_t		*lnodv[MAX_DOMAINS+2], *save_node[MAX_DOMAINS+2];
	...
	setp(PV_STR,"0", 0);	/* initial r_status field */
	relnum = rnum_alloc();
	setp(PV_STR,rnum_convert(relnum), 0);
	setp(PV_STR,"count", 0);	/* domain 1 - count field per BY value */
	setp(PV_STR,"i4", 0);	/* format of count field */
	...
	bymap = varfind(byhead->left, (qtree_t *)NULL);
	...
	** or aggregate is multi-var, then special processing is done */
	...
	initp();
	setp(PV_STR, rnum_convert(relnum), 0);
	setp(PV_STR, "hash", 0);	/* modify the empty rel to hash */
	setp(PV_STR, "num", 0);	/* code to indicate numeric domain names */
	nums[1] = '\0';
	for (i = 0; i < bydoms; i++) {
		nums[0] = i + 2;
		setp(PV_STR, nums, 0);
