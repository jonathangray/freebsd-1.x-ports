...
*/
void
setvar(register qtree_t *tree, int var, tid_t *intid, char *tuple)
{
		...
		if ((tree=ckvar(tree))->sym.value.sym_var.varno == var) {
			...
			if (tree->sym.value.sym_var.attno)
				tree->sym.value.sym_var.valptr =
				    (ANYTYPE *) (tuple + readopen(var)->d_off[(int) tree->sym.value.sym_var.attno]);
			else
...
*/
void
clearvar(register qtree_t *tree, register int var)
{
	if (tree == NULL)
		return;
