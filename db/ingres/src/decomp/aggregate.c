...
**	cannot exceed MAX_TUP_SIZE and the number of domains cannot exceed MAX_DOMAINS-1
*/
int
chkwidth(qtree_t *aop, int *widthp, int domno)
{
	register int	width;
	...
	if (width > MAX_TUP_SIZE || domno > MAX_DOMAINS - 1)
		return (1);
...
int
sameafcn(qtree_t *q1, qtree_t *q2)
{

	register qtree_t	*t1, *t2;
	register int	len;
	int		type;
...
int
sameagg(qtree_t *aghead, qtree_t *newaop, int agg_depth)
{
	register qtree_t	*agg, *newa;
	register int	i;
...
*/
int
varfind(qtree_t *root, qtree_t *aghead)
{
...
int
modtree(qtree_t **pnode, qtree_t **lnodv, int *replmap)
{
	register qtree_t	*tree;
	register int	vars, i;
	qtree_t		*afcn;
...
#endif
			vars = varfind(tree, (qtree_t *)NULL);
...
void
chklink(qtree_t *root)
{
	register qtree_t	*r, *b, *last;
...
void
opt_bylist(struct agglist *alist, qtree_t *root)
{
	register struct agglist	*al;
	register qtree_t		*agg;
	register struct hitlist	*hl;
	qtree_t			**tpr, *tree, *lnodv[MAX_DOMAINS+2];
	...
	for (al = alist; (agg = al->agpoint) && agg != root; al++) {
		if (agg->sym.type == AGHEAD && agg->left->sym.type == BYHEAD &&
			...
			if ((treemap & varfind(agg->left->left, (qtree_t *)NULL)) == 0)
				continue;
			...
			*/
			if (usedmap && ((vars & usedmap) == 0)) {
				...
				for (hl = &hlist[0]; (tpr = hl->trepr) != 0 ; hl++) {
					/* get bydomain number */
					i = hl->byno;
...
*/
void
findagg(qtree_t **nodep,  qtree_t *agf)
{
	register qtree_t		*q, *f;
...
agspace(qtree_t *aop)
{
	register qtree_t	*a, *r;
	register int	length;

	a = aop;
	length = a->sym.value.sym_op.opfrml & I1MASK;
	r = (qtree_t *) need(De.de_qbuf, length + QT_HDR_SIZ);
	r->left = r->right = 0;
...
int
checkagg(qtree_t *aghead1, qtree_t *aop1, qtree_t *aghead2, qtree_t *aop2)
{
	register qtree_t	*aop_1, *aop_2, *agg1;
	int		ok;
...
*/
void
aggregate(qtree_t *root)
{
	struct agglist	alist[MAX_QRY_AGGS + 1];
	qtree_t		*rlist[MAX_QRY_AGGS + 1];
	struct agglist	*al, *al1;
	register qtree_t	*agg, *aop1, *r;
	qtree_t		*aop, *agg1;
	...
	for (; (agg = al->agpoint) != 0 ; al++) {
		...
		for (al1 = al + 1; (agg1 = al1->agpoint) != 0 ; al1++) {
