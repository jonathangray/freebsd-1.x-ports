...
int
calln(int entpt, int (*errfn)(void))
{
	resp_t			*rp;
	pb_t			pb;
#ifdef xMONITOR
	...
	*/
...
#ifdef xMONITOR
	markperf(savemon);
	rp->resp_time = mon.mon_utime + mon.mon_stime;
#endif xMONITOR
	...
	if (tTf(5, 3)) {
		...
		printf("\tresp %7d\ttime %7ld\ttups %6ld\n",
			rp->resp_resp, rp->resp_time, rp->resp_tups);
		printf("\tpread %6ld\tpwrit %6ld\n",
			rp->resp_pread, rp->resp_pwrit);
	}
#endif
...
void
call_setup(register pb_t *ppb, int state, int (*errfn)(void))
{
#ifdef xCTR1
	if (tTf(5, 5)) {
