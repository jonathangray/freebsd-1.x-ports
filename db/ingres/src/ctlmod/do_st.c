...
int
do_st(register pb_t *ppb, int pc, paramv_t *pv)
{
	register state_t	*s;
	register int		i;
	int			rtval;
	...
	switch (s->st_type) {
	  ...
	  case ST_LOCAL:
		...
		*/
		...
		if (BITISSET(PB_FRFR, ppb->pb_stat) && !BITISSET(ST_EXTERN, s->st_stat))
			syserr("do_st: restricted state %d", i);
		if (ppb->pb_from == Cm.cm_myproc)
			BITSET(PB_EOF, ppb->pb_stat);
