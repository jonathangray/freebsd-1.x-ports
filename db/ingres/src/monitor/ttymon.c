...
/* XXX - added for Pathname and Fileset */
#include <aux.h>
#include <batch.h>
/* XXX - added for Pathname and Fileset */
...
tm_init(int argc, char **argv)
{
	...
	if (Nodayfile == 0 && (Qryiop = fopen(ztack(ztack(Pathname, "/files/dayfile"), MAJOR_VERSION), "r")) != NULL) {
...
tm_intr(int typ)
{
	...
	lseek(stdin->_file, (off_t) 0, SEEK_END);
	Newline = Prompt = TRUE;
	Nodayfile = Userdflag;
	Oneline = FALSE;
	Idepth = 0;
...
proc_err(pb_t *ppb, int pc, paramv_t *pv)
{
	...
	Error_id = err;
	fatal = !BITISSET(PB_INFO, ppb->pb_stat);
		...
		*p = '\0';
		...
		for (;;) {
			...
			if (c == '%') {
				...
				switch (pv[c].pv_type) {
				  case PV_STR:
					for (p = pv[c].pv_val.pv_str; (c = *p) != 0 ; p++)
						xputchar(c);
					continue;
...
{
#ifdef DEBUG
	printf("%d: fake closecatalog (ttymon)\n", getpid());
#endif
}
