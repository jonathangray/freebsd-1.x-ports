...
macro_t	*Machead	= &Macprims[0];	/* head of macro list */
...
typedef struct env {
	...
	buf_t	*pbuf;		/* peek buffer */
	buf_t	*mbuf;		/* macro buffer */
...
int macscan(macro_t *mac);
macro_t *macmlkup(char *name);
void macsubs(macro_t *mac);

void macpflush(env_t *env);
...
*/
void
macinit(int (*rawget)(char **val), char **rawpar, int endtrap)
{
	static env_t	env;
	register env_t	*e;
	register env_t	*f;
...
*/
int
macparam(char mode, char name, char delim)
{
	register char		c;
	register env_t	*e;
	buf_t		*b;
...
*/
void
macnewev(int (*rawget)(char **val), char **rawpar)
{
	register env_t	*e;

	e = (env_t *) bufalloc(sizeof(*e));
	e->rawget = rawget;
	e->rawpar = rawpar;
	e->nexte = Macenv;
	e->newline = 1;
...
*/
char *
mactcvt(int raw, char *paramdefined)
{
	register int		c;
	buf_t		*b;
	register char		d;
...
macremove(char *name)
{
	...
	while (name == 0 ? ((m = Machead)->substitute > (char *) NPRIMS) : ((m = macmlkup(cname)) != 0)) {
		...
		/* remove macro from list */
		*mp = m->nextm;
		buffree((char *) m->template);
		buffree((char *) m->substitute);
		buffree((char *) m);
	}
	buffree((char *) cname);
}
...
*/
void
macdefine(char *template, char *subs, int raw)
{
	register env_t	*e;
...
*/
int
macmmatch(char *name, char *temp, int flag)
{
	register char	ac;
	register char	bc;
	char		*ap, *bp;
...
char *
macsstr(int from, int to, char *string)
{
	register int	f;
	int		l;
	register char	*s;
...
macmocv(char *m)
{
	...
	if (lastbuf != 0) {
		buffree((char *) lastbuf);
		lastbuf = 0;
	}

	if (p <= (char *) NPRIMS) {
		/* we have a primitive */
		/* trying to avoid writable strings - agc */
		(void) strcpy(p = buffer, "Primitive xxx");
		itoa((int) m, &p[10]);
		return(p);
	}
...
macprim(int n)
{
	...
	switch (n) {
	  ...
	  case 8:	/* {ifsame; $a; $b; $t; $f} */
		if (strcmp(macplkup('a'), macplkup('b')) == 0)
			return (macplkup('t'));
		else
			return (macplkup('f'));
