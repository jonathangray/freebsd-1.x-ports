...
int
bndxsearch(desc_t *d, struct tup_id *tidx, char *tuple, int mode, int keyok, struct tup_id *path)
{
		...
		do {
			...
			if (top >= SEARCHFUDGE)  /* we are past breakeven */ {
				...
				while (top != bottom)	/* binary search */ {
					...
					p = get_addr(tid) + sizeof(tid_t);
					for (j = 0; dno = relparm.keydno[j]; j++) {
						keylen = d->d_len[dno] & I1MASK;
						printf("data: ");
						printatt(d->d_fmt[dno], keylen, p);
						printf("| compared with: ");
						printatt(d->d_fmt[dno], keylen, &tuple[d->d_off[dno]]);
						printf("|\n");
						if (i = icompare(&tuple[d->d_off[dno]], p, d->d_fmt[dno], keylen))
							break;
						p += keylen;
					}
				...
				for (tid->line_id = 0; (tid->line_id & I1MASK) < Acc_head->am_nextline; tid->line_id++) {
					...
					for (i = 0; dno = relparm.keydno[i]; i++) {
						...
						printatt(d->d_fmt[dno], keylen, &tuple[d->d_off[dno]]);
						printf("|\n");
						if (j = icompare(&tuple[d->d_off[dno]], p, d->d_fmt[dno], keylen))
							break;
