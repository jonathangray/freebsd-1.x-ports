 ...
 *	and to make the functions use ANSI C type arguments.
 *
 *	Subsequent changes to:
 *	- make regexps into SREs
 *	- include \<\>, \s\S, \w\W, \p\P, \d\D recognition
 *	- added {min,max} recognition of strings
 *	- have an access function for the size of the compiled expression
 *	- made it work with GNU autoconf
 */

/*
 * SREcomp and SREexec -- SREsub and SREerror are elsewhere
 *
 ...
 * that SREcomp() supplies a regmust only if the r.e. contains something
 * potentially expensive (at present, the only such thing detected is * or +
 * at the start of the r.e., which can involve a lot of backup).  Regmlen is
 * supplied because the test in SREexec() needs it and SREcomp() is computing
 * it anyway.
 */

/*
 * Structure for SRE "program".  This is essentially a linear encoding
...
#define	ANY	3	/* no	Match any one character (not \n). */
...
#define WHITESP 14      /* node Match any whitespace character */
#define NWHITESP 15     /* node Match any nonwhitespace character */
...
#define MINMAX	22	/* node Match this thing (min, max) times */
...
#define	FAIL(m)		{ SREerror(m); return(NULL); }
#define	ISMULT(c)	((c) == '*' || (c) == '+' || (c) == '?' || (c) == '{')
#define	META		"^$.[()|?+*\\{"

#ifndef isident
#define isident(x)	((x) == '_' || isalnum(x))
#endif /* !isident */
...
/*
 * Global work variables for SREcomp().
 */
...
/*
 * Forward declarations for SREcomp()'s friends.
 */
#ifndef STATIC
 ...
 */
/*
 * Global work variables for SREexec().
 */
 ...
 * Forwards.
 */
STATIC int	regtry(SRE *prog, char *string);
...
/*
 * SREsub
 * agc - added here for completeness - all SRE functions in one file.
 *
 *	Copyright (c) 1986 by University of Toronto.
 *	Written by Henry Spencer.  Not derived from licensed software.
 *
