...
SCCSID(@(#)seq_atts.c	8.1	12/31/84)
...
**	the same order as they were created in; that is "a_id" order.
**
...
void
seq_init(register desc_t *a, register desc_t *r)
{
	register int		i;
	attr_t	attkey;

	clearkeys(a);
	ingres_setkey(a, &attkey, r->d_r.r_id, ATTRELID);
	ingres_setkey(a, &attkey, r->d_r.r_owner, ATTOWNER);
	if ((i = find(a, EXACTKEY, &Seq_tdl, &Seq_tdh, &attkey)) != 0)
...
int
seq_attributes(register desc_t *a, register desc_t *r, register attr_t  *atup)
{
	int	i, nxttup;

	if (Seq_next > r->d_r.r_attrc)
