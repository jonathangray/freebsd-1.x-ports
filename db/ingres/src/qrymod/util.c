...
void
qmerror(int errno, int qmode, int vn, ...)
{
	register char		*x1;
	register char		*x2;
	char			xbuf[MAX_NAME_SIZE + 1];
	register int		i;
	va_list			vp;
	...
	i = vn;
	if (i >= 0)
		smove(trim_relname(Qt.qt_rangev[i].rngvdesc->d_r.r_id),
		      x2 = xbuf);

	/* issue the error message and exit */
	va_start(vp, vn);
	error(errno, x1, x2, vp);
	va_end(vp);
	syserr("qmerror");
}
