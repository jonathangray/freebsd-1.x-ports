...
*/
void
vrscan(qtree_t *root, qtree_t *vtree)
{
	register qtree_t	*t;
	register qtree_t	*v;
	int		i;
...
view(qtree_t *root)
{
	...
	while (viewfound) {
		...
		for (vn = 0; vn < MAX_VARS + 1; vn++) {
			...
			if (!BITISSET(S_VIEW, Qt.qt_rangev[vn].rngvdesc->d_r.r_status))
				continue;
#ifdef xQTR1
			if (tTf(30, 3))
				printf("view vn %d: %.12s\n", vn,
				    Qt.qt_rangev[vn].rngvdesc->d_r.r_id);
#endif

			vtree = gettree(Qt.qt_rangev[vn].rngvdesc->d_r.r_id,
					Qt.qt_rangev[vn].rngvdesc->d_r.r_owner,
					mdVIEW, 0, FALSE);
#ifdef xQTR3
			if (tTf(30, 5))
				treeprn(vtree, "Viewdef");
#endif
