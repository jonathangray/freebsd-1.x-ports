...
*/
void
dopro(int varno, qtree_t *root, int qmode, int *byset)
{
	int		qset[8];
	int		uset[8];
	int		aset[8];
...
/*	if (noperm == 0)
	...
	ingres_setkey(&Prodes, &prokey, Qt.qt_rangev[vn].rngvdesc->d_r.r_id, PRORELID);
	ingres_setkey(&Prodes, &prokey, Qt.qt_rangev[vn].rngvdesc->d_r.r_owner, PRORELOWN);
	...
	while ((i = get(&Prodes, &lotid, &hitid, &protup, TRUE)) == 0) {
		...
		if (protup.p_tree >= 0) {
			p = gettree(Qt.qt_rangev[vn].rngvdesc->d_r.r_id,
				    Qt.qt_rangev[vn].rngvdesc->d_r.r_owner,
				    mdPROT, protup.p_tree, FALSE);
#ifdef xQTR2
			if (tTf(51, 6))
				treeprn(p, "Protection Clause");
#endif
			p = trimqlend(p->right);
#ifdef xQTR3
			...
			for (j = 0; j < 4; j++) {
				...
				if (quals[j].q_qual == NULL)
					...
					quals[j].q_qual = qm_tree(quals[j].q_qual, p, OR, 0, 0);
			}
		}
		else
			noqual = TRUE;
		...
		for (i = 0; i < 4; i++)
			if (quals[i].q_qual != NULL)
				if (pqual == NULL)
					...
					pqual = qm_tree(pqual, quals[i].q_qual, AND, 0, 0);
		pqual = qm_tree(pqual, qm_tree(NULL, NULL, QLEND, 0, 0), AND, 0, 0);
		appqual(pqual, t);
...
*/
void
makedset(int vn, qtree_t *tree, int *uset, int *rset, int *aset, int *qset)
{
	register qtree_t	*t;
	register int	i;
	int		byset[8];
...
*/
int
prochk(int inbit, int *domset, protect_t *protup)
{
...
#ifdef xQTR1
	if (tTf(55, 0)) {
		printf("->prochk, inbit=%o, p_opset=%o\n", inbit, p->p_opset);
		pr_set(d, "domset");
		pr_set((int *) p->p_domset, "p_domset");
	}
#endif
...
protect(qtree_t *root)
{
	...
	*/
	...
	for (vn = 0; vn < MAX_VARS + 1; vn++) {
		...
		if (bequal(d->d_r.r_owner, Usercode, USERCODE_SIZE))
			continue;

		/* check for "no restriction" bit asserted (= clear) */
		if (!BITISSET(S_PROTALL, d->d_r.r_status))
			continue;
		if (!BITISSET(S_PROTRET, d->d_r.r_status) &&
...
proappl(protect_t *protup)
{
	...
	if (!bequal("  ", p->p_user, USERCODE_SIZE)) {
		if (!bequal(p->p_user, Usercode, USERCODE_SIZE)) {
#ifdef xQTR2
			if (tTf(54, 0))
				printf("  ~user\n");
#endif
	...
	(void) time(&t);
	tp = localtime(&t);
	mtime = (tp->tm_hour * 60 * 60) + (tp->tm_min * 60) + tp->tm_sec;

	if (p->p_tbegin > mtime || p->p_tend < mtime) {
#ifdef xQTR2
		if (tTf(54, 0))
			printf("  ~tod\n");
#endif
	...
	if (p->p_dbegin > tp->tm_wday || p->p_dend < tp->tm_wday) {
#ifdef xQTR2
		if (tTf(54, 0))
			printf("  ~dow\n");
#endif
