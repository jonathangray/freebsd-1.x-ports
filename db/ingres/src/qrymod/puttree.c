...
*/
int
puttree(qtree_t *root, char *trelid, char *towner, char ttype)
{
	struct tree	treekey;
	struct tree	treetup;
	...
	*/
	...
	setrll(A_SLP, &Treedes.d_tid, M_EXCL);

	ingres_setkey(&Treedes, &treekey, trelid, TREERELID);
	ingres_setkey(&Treedes, &treekey, towner, TREEOWNER);
	ingres_setkey(&Treedes, &treekey, &ttype, TREETYPE);
	for (treeid = 0;; treeid++) {
		ingres_setkey(&Treedes, &treekey, &treeid, TREEID);
...
*/
int
relntrwr(char *ptr, int len, char *treerelid, char *treeowner, int treetype, int treeid)
{
	...
	if (p == NULL) {
		switch (l) {
		  case 0:
			clr_tuple(&Treedes, (char *) &treetup);
			bmove(treerelid, treetup.treerelid, MAX_NAME_SIZE);
			bmove(treeowner, treetup.treeowner, sizeof(treetup.treeowner));
	...
	while (l-- > 0) {
		*tptr++ = *p++;
		...
		for (i = 0; i < sizeof(treetup.treetree) ; i++)
			((char *)treetup.treetree)[i] = ' ';
	}
