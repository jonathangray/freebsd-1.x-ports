...
qtree_t *
qscan(qtree_t *root, int vn, int an)
{
	register qtree_t	*t;
	register qtree_t	*u;
...
**			Mostly, changes the handling of tid_t type
...
*/
void
subsvars(qtree_t **proot, int vn, qtree_t *transtree, int vmode)
{
	register qtree_t	*t;
	register qtree_t	*v;
...
#ifdef xQTR3
	if (tTf(32, 0))
		printf("subsvars: vn %d root %p transtree %p\n", vn, t, v);
#endif
	...
	if (t->sym.type == RESDOM && t->sym.value.sym_resdom.resno == 0) {
		...
		t = t->right;
		if (t->sym.type != VAR ||
		    t->sym.value.sym_var.attno != 0 ||
		    t->sym.value.sym_var.varno != vn)
	...
	if (v == NULL) {
		if (vmode == mdVIEW) {
			syserr("subsvars: attno %d",
						t->sym.value.sym_var.attno);
		} else if (vmode == mdAPP) {
			v = makezero(INT_CONST);
		}
	} else {
		v = treedup(v);
	}
