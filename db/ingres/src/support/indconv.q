...
##	char		*Usercode;
	struct {
		char	relname[MAX_NAME_SIZE + 1], indname[MAX_NAME_SIZE + 1], relsp[10];
		char	domname[7][MAX_NAME_SIZE + 1], modseq[7];
	}	Indinfo[100];
...
##	retrieve (iname = r.r_id, rspec = r.r_spec)
##		where r.r_indexed < 0 and r.r_indexed != -2 and r.r_owner = Usercode
##	{
...
##	}
##	range of x is indices
##	retrieve (pname = x.i_relname, iname = x.i_index)
##		where x.i_owner = Usercode
##	{
		for (i = 0; i < cnt; i++) {
...
##	retrieve (iname = a.a_rel, aname = a.a_name, aid = a.a_id,
##			akey = a.a_iskey)
##		where a.a_owner = Usercode
##	{
		for (i = 0; i < cnt; i++) {
			if (sequal(iname, Indinfo[i].indname)) {
				if (akey > 0)
