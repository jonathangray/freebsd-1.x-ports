...
char*db;
	char	*path;
char name[MAX_NAME_SIZE];int indx;
int atts;
int spec;
char iname[MAX_NAME_SIZE];
...
{IIwrite("retrieve(Usercode=usercode)");IIsetup();while(IIn_get(0)){IIn_ret(Usercode,3);if(IIerrtest())continue;
}IIflushtup(0);}
{IIwrite("range of r=relation");IIsync(0);}
	...
	*/
	relptr = relations;
{IIwrite("retrieve(name=r.r_id,spec=r.r_spec,indx=r.r_indexed,atts=r.r_attrc)where r.r_owner=");IIcvar(Usercode
,3,0);IIwrite(" and mod(r.r_status,2)!=1 and r.r_indexed>=0 and r.r_id!=\"_SYS*\"");IIsetup();while(IIn_get(0)){
IIn_ret(name,3);IIn_ret(&spec,6);IIn_ret(&indx,6);IIn_ret(&atts,6);if(IIerrtest())continue;
...
{IIwrite("range of i=indices");IIsync(0);}
	n = i;
...
{IIwrite("retrieve(iname=i.i_index,spec=i.i_indrelspec,idomv1=i.idom1,idomv2=i.idom2,idomv3=i.idom3,idomv4=i.idom4");
IIwrite(",idomv5=i.idom5,idomv6=i.idom6)where i.i_relname=");IIcvar(name,3,0);IIwrite(" and i.i_owner=");IIcvar(
Usercode,3,0);IIwrite("");IIsetup();while(IIn_get(0)){IIn_ret(iname,3);IIn_ret(&spec,6);IIn_ret(&idomv1,6);IIn_ret(
&idomv2,6);IIn_ret(&idomv3,6);IIn_ret(&idomv4,6);IIn_ret(&idomv5,6);IIn_ret(&idomv6,6);if(IIerrtest())continue;
...
char aname[MAX_NAME_SIZE];int domain,length,extra;
char format[1];


	i = xcount = 0;
{IIwrite("retrieve(aname=a.a_name,domain=a.a_id,format=a.a_fmt,length=a.a_len,extra=a.a_iskey)where a.a_rel=");
IIcvar(name,3,0);IIwrite(" and a.a_owner=");IIcvar(Usercode,3,0);IIwrite("");IIsetup();while(IIn_get(0)){IIn_ret(
aname,3);IIn_ret(&domain,6);IIn_ret(format,3);IIn_ret(&length,6);IIn_ret(&extra,6);if(IIerrtest())continue;
