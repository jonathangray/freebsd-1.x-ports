...
##	retrieve ( sysmod = r.r_spec ) where r.r_id = "_rtempv8"
...
##	create _rtempv8(r_id=c12, r_owner=c2, r_spec=i1, r_indexed=i1, 
##			r_status2=i2, r_status=i2, r_savetime=i4, r_tupc=i4, 
##			r_attrc=i2, r_width=i2, r_primc=i4, r_free=i4, 
##			r_modtime=i4, r_dim=i2)


##	retrieve ( user = r.r_owner ) where r.r_id = "relation"
	User[0] = user[0];
	User[1] = user[1];
	User[2] = '\0';
...
##	retrieve into _rtempv81 (r.all, r_dim = 0)
##	range of t is _rtempv81
##	append _rtempv8 (t.all)
##	destroy _rtempv81
##	range of t is _rtempv8
##	delete t where t.r_id = "_rtempv8" or t.r_id = "_rtempv81"
##	replace t ( r_savetime = 0, r_status = 275, r_spec = 11) where t.r_id = "rdelim"
##	replace t ( r_tupc = t.r_tupc -1, r_attrc = t.r_attrc + 1, r_width = t.r_width + 2) where
##		t.r_id = "relation"


##	retrieve into _atempv8 (a.all)
##	append _atempv8(a_rel="relation",a_owner=user, a_id=14, a_name="r_dim", a_off=44, a_fmt="i", a_len=2, a_iskey=0)

##	range of t is _atempv8
##	delete t where t.a_rel = "_rtempv8" or t.a_rel = "_rtempv81" or t.a_rel = "_atempv8"

	sysmod = 0;

##	retrieve ( sysmod = r.r_spec ) where r.relid = "relation" and r.r_owner = user

	if ( sysmod != 5 ) {
##		modify _rtempv8 to hash on r_id
##		modify _atempv8 to hash on a_rel,a_owner
	}

	user[0] = user[1] = '\0';
##	retrieve ( user = r.r_owner ) where r.r_id = "_atempv8"
