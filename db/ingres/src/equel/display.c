...
struct disp_node *
enter_display(struct display *disp, char *string)
{
...
void
w_display(struct display *disp)
{
	register struct disp_node	*n;

	for (n = disp->disp_first; n; n = n->d_next) {
		switch (equel_cmap (n->d_elm [strlen(n->d_elm) - 1])) {
...
void
eat_display(struct display *disp, char left_ch, char right_ch)
{
