...
*/
void
par_error(int num, int result, char *a, char *b, char *c)
{
	register char	*buf;
	char		buff[30];
...
neederr(int errnum)
{
	par_error(errnum, WARN, (char *) NULL, (char *) NULL, (char *) NULL);
}

void
yyerror(char *errmessage)
{
#ifdef	xPTR1
	tTfp(60, 6, "yyerror: an error from yacc itself\n");
#endif

	if (strcmp(errmessage, "syntax error") == 0)
		par_error(SYMERR, WARN, 0, 0, 0);
	else
		par_error(YOVRFLOW, WARN, 0, 0, 0);
}
