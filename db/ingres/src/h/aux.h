...
**		@(#)aux.h	8.2	2/8/85
*/
#ifndef INGRES_AUX_H_
#define INGRES_AUX_H_
...
*/
typedef struct accessparam {
	short	mode;		/* mode of rel, NOKEY, RANGEKEY, EXACTKEY */
	short	sec_index;	/* TRUE if relation is a secondary index. */
	char	keydno[MAX_DOMAINS + 1];
} acc_param_t;
...
struct desxx {
	...
	desc_t	*cach_alias;	/* alias for above descriptor */
};

/*
**  Relation status bits
**
**	These bits are in the relation relation, in the "r_status"
...
**	be at most MAX_LINE_SIZE bytes long.  buffers designed for holding
**	such info should be decleared as char buf[MAX_LINE_SIZE + 1] to allow
